...
The next permitted work includes the separately versioned, read-only `phase15-v3-canary-readiness-v1` accelerated audit. Its sample rule reuses the pre-existing uncertainty principle rather than a magic trade count; its new calibration intercept/slope reliability rule is frozen before those diagnostics are read. This audit cannot activate money. Geographic compliance remains a separate mandatory gate, and geographic restrictions may not be bypassed. V3 paper observation and V4 prospective collection continue unchanged.
