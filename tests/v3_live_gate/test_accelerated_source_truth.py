...
def test_phase15_v3_readiness_and_geography_pass_for_one_dollar_canary() -> None:
    ...
    assert candidate["status"] == "GEOBLOCK_PASS_RUNNING_NO_TRADING_SOFTWARE"
    assert candidate["vm_name"] == "bp-v3-canary-exec"

    executor_geo = gate["execution_host_geoblock"]
    assert executor_geo["status"] == "pass"
    assert executor_geo["blocked"] is False
    assert executor_geo["country"] == "ZA"
    assert executor_geo["region"] == "GP"
