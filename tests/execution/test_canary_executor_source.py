...
def test_executor_has_atomic_one_submit_guard_and_ten_dollar_cap() -> None:
    text = SOURCE.read_text(encoding="utf-8")
    assert 'CANARY_MAX_NOTIONAL_USD = Decimal("10.00")' in text
    assert "os.O_EXCL" in text
    assert "first-submit.reserved" in text
    assert "ten-dollar canary submit already consumed" in text
