...
def test_one_dollar_canary_policy_is_tiny_and_frozen() -> None:
    policy = _policy()
    assert CANARY_TARGET_NOTIONAL_USD == Decimal("1.00")
    assert CANARY_MAX_TOTAL_EXPOSURE_USD == Decimal("1.00")
    assert CANARY_MAX_DAILY_LOSS_USD == Decimal("2.00")
    assert CANARY_MAX_CONSECUTIVE_LOSSES == 2
    assert CANARY_MIN_EDGE == Decimal("0.075")
    assert policy.max_trade_size_usd == Decimal("1.00")
    assert policy.max_total_exposure_usd == Decimal("1.00")
    assert policy.max_daily_loss_usd == Decimal("2.00")
    ...
    assert V3_LIVE_CANARY_EXECUTION_VERSION == "live-execution-v3-canary-v1"
