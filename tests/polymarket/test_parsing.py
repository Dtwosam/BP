...
def test_parse_horizon_slug_uses_epoch_as_market_window_start() -> None:
    seconds, start = parse_horizon_slug("btc-updown-5m-1787260500")

    assert seconds == 300
    assert start == datetime(2026, 8, 20, 21, 15, tzinfo=UTC)
...
def test_parse_5m_market_maps_tokens_by_outcome_label_not_array_position() -> None:
    ...
    assert market.window_start_at == datetime(2026, 8, 20, 21, 15, tzinfo=UTC)
    assert market.window_end_at == datetime(2026, 8, 20, 21, 20, tzinfo=UTC)
    assert market.up_token_id == (
        "60212198060782700007060418357570727096093887222836004779804011735224977276165"
    )
    assert market.down_token_id == (
        "852370992865798591039298383758918055714201693624021683582242719293344349868"
    )
