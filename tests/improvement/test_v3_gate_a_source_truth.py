...
def test_project_state_records_gate_a_pass_and_frozen_v3_preregistration() -> None:
    ...
    assert state["source_of_truth_version"] == "0.14.177"
...
def test_canonical_docs_record_gate_a_pass_and_preregistration_handoff() -> None:
    ...
    assert "v4 regime-aware feature collector" in build.lower()
...
def test_current_handoff_points_to_fail_closed_storage_recovery_sequence() -> None:
    start = _text("START-HERE.md")
    build = _text("docs/BUILD-ORDER.md")

    start_next = start.split("## Immediate next task", 1)[1].lower()
    assert "historical production pass" in start_next
    assert "active after concurrent-partition-retirement rollout pass" in start_next
    ...
    assert "frozen v4 gate b v1 future cohort" in start_next
    ...
    assert "active after concurrent-partition-retirement rollout pass" in build_next
