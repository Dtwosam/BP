...
def test_phase14_observation_source_truth_preserves_read_only_boundary() -> None:
    ...
    assert observation["database_read_only_hardening_status"] == "MERGED_MAIN_GREEN_NOT_PRODUCTION_RUN"
    ...
    assert (
        observation["database_read_only_hardening_final_head"]
        == "3747f4bbf13ab3fec12b4b08c10df52c0b0dbbec"
    )
    assert observation["database_read_only_hardening_final_ci_run_id"] == 35608740215
    assert (
        observation["database_read_only_hardening_merge_commit"]
        == "cc646933c82c4a0b9a613d98a769c07aca9cfe49"
    )
    assert observation["database_read_only_hardening_post_merge_ci_run_id"] == 35609242166
