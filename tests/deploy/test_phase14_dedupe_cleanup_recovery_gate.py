...
HELPER = ROOT / "scripts/deploy/phase14_dedupe_cleanup_recovery_gate_cloudshell.sh"
CI = ROOT / ".github/workflows/ci.yml"


def test_dedupe_cleanup_recovery_gate_binds_exact_minimal_production_candidate() -> None:
    content = HELPER.read_text(encoding="utf-8")

    for required in (
        "PHASE14_DEDUPE_CLEANUP_RECOVERY_HELPER_HEAD",
        "71b33d3beaba4a11ef93e7c5bde1c517323f3440",
        "ops/phase14-dedupe-cleanup-recovery-candidate",
        ...
        "PHASE14_DEDUPE_CLEANUP_RECOVERY_APPROVAL",
        "I_APPROVE_PHASE14_DEDUPE_CLEANUP_RECOVERY",
    ...
    assert "src/bp_engine/storage/partitioned_raw.py" not in content


def test_dedupe_cleanup_recovery_gate_requires_existing_four_worker_safe_runtime() -> None:
    content = HELPER.read_text(encoding="utf-8")
...
def test_dedupe_cleanup_recovery_gate_proves_fix_under_active_recorder_load() -> None:
    content = HELPER.read_text(encoding="utf-8")
    ...
    for required in (
        ...
        "/var/lib/bp/evidence",
        "phase14-dedupe-cleanup-recovery-",
        "PHASE14_DEDUPE_CLEANUP_RECOVERY_GATE=PASS",
    ):
        assert required in content
...
def test_dedupe_cleanup_recovery_gate_rolls_back_checkout_and_keeps_recorder_stopped() -> None:
    content = HELPER.read_text(encoding="utf-8")

    for required in (
        "PHASE14_DEDUPE_CLEANUP_RECOVERY_ROLLBACK=START",
        "PHASE14_DEDUPE_CLEANUP_RECOVERY_ROLLBACK=COMPLETE",
...
def test_ci_syntax_checks_dedupe_cleanup_recovery_gate() -> None:
    ci = CI.read_text(encoding="utf-8")
    assert (
        "bash -n scripts/deploy/phase14_dedupe_cleanup_recovery_gate_cloudshell.sh"
        in ci
    )
