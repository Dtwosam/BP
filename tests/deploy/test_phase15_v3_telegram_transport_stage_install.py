...
def test_transport_stage_install_surfaces_blocked_preflight_reports() -> None:
    text = INSTALL.read_text(encoding="utf-8")
    publisher_marker = (
        "'^PHASE15_V3_TELEGRAM_TRANSPORT_PUBLISHER_INSTALL_PREFLIGHT=PASS    text = INSTALL.read_text(encoding="utf-8")
    for marker in (
        "gcloud compute scp",
        "python3 -m venv",
        "--only-binary=:all:",
...
"
    ...
    executor_marker = "'^PHASE15_V3_TELEGRAM_TRANSPORT_INSTALL_PREFLIGHT=PASS    text = INSTALL.read_text(encoding="utf-8")
    for marker in (
        "gcloud compute scp",
        "python3 -m venv",
        "--only-binary=:all:",
...
def test_transport_stage_install_makes_runtime_venvs_service_user_usable() -> None:
    ...
    for marker in (
        ...
        assert marker in executor_text
"
    assert publisher_marker in text
    assert executor_marker in text
    assert 'cat "$PUBLISHER_PREFLIGHT" >&2' in text
    assert 'cat "$EXEC_PREFLIGHT" >&2' in text
