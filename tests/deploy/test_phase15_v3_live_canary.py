...
def test_persistent_prepare_start_requires_explicit_scope_and_health_only() -> None:
    ...
    for marker in (
        ...
        'stage.get("status") == "PRODUCTION_ACTIVE_WAITING_FOR_FRESH_TELEGRAM_APPROVAL"',
        'stage.get("activation_pass") is True',
        'stage.get("executor_safe_idle") is True',
        'activation.get("status") == "ACTIVATED_WAITING_FOR_FRESH_TELEGRAM_APPROVAL"',
        'activation.get("authorization_consumed") is True',
        'activation.get("telegram_approval_performed") is False',
        'activation.get("executor_armed") is False',
        'activation.get("order_submission_performed") is False',
    ...
    assert (
        "--authorized-submission-attempt-limit "
        "${BP_PHASE15_PREPARE_AUTHORIZED_SUBMISSION_ATTEMPT_LIMIT}"
    ) in unit_text
    assert (
        "--authorized-accepted-order-limit "
        "${BP_PHASE15_PREPARE_AUTHORIZED_ACCEPTED_ORDER_LIMIT}"
    ) in unit_text
    ...
    assert (
        "authorized_submission_attempt_limit="
        "args.authorized_submission_attempt_limit"
    ) in runner_text
    assert (
        "authorized_accepted_order_limit="
        "args.authorized_accepted_order_limit"
    ) in runner_text
    for forbidden in (
        '{"action":"submit"}',
        "post_order",
