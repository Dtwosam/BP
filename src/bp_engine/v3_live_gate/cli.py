...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Read-only frozen V3 live-gate reassessment"
    )
...
def _latest(
    ...
    return sorted(
        latest.values(),
        key=lambda row: (row[timestamp], str(row[key])),
    )
    ...
    *,
    ...
    try:
        with engine.connect() as connection:
            ...
            if read_only != "on":
                raise RuntimeError(
                    "V3 live-gate reassessment requires a read-only PostgreSQL session"
                )
        ...
        latest_settlements = _latest(
            settlements,
            "paper_order_id",
            "settled_at",
        )
        latest_evaluations = _latest(
            evaluations,
            "prediction_id",
            "evaluated_at",
        )
        ...
        for settlement in latest_settlements:
            ...
            if (
                ...
                or cost != settlement["total_fill_cost"]
            ):
                violations += 1
