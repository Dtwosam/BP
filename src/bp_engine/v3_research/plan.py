...
class V3PlanIntegrityError(RuntimeError):
    """Raised when the frozen V3 feature-only plan cannot be constructed safely."""
...
def _config_payload(config: V3GateBConfig) -> dict[str, object]:
    return {
    ...
    *,
    ...
    for condition_id, condition_rows in grouped.items():
        ...
        if len(offsets) != len(set(offsets)):
            raise V3PlanIntegrityError(f"{condition_id} has duplicate feature offsets")
...
def _embargo_earlier_side(
    ...
    *,
    ...
    if len(markets) < embargo_markets:
        raise V3PlanIntegrityError("partition is too small for frozen embargo")
...
def _build_fold(
    ...
    *,
    ...
    purged = sorted(set(train_purged + validation_purged + test_purged))
    embargo = train_embargo + validation_embargo
    payload: dict[str, object] = {
        "index": index,
        "train": _partition_payload(start=train_start, end=train_end, markets=train),
        "validation": _partition_payload(
            start=validation_start,
            end=validation_end,
            markets=validation,
        ),
        "test": _partition_payload(start=test_start, end=test_end, markets=test),
...
def _build_final_partition(
    ...
    *,
    ...
    purged = sorted(set(train_purged + validation_purged + holdout_purged))
    embargo = train_embargo + validation_embargo
    payload: dict[str, object] = {
...
def _assert_partition_integrity(
    *,
    ...
    if len(ordinary_test_ids) != len(set(ordinary_test_ids)):
        raise V3PlanIntegrityError("ordinary test membership is not unique")

    final_ids = set(final["holdout_condition_ids"])
    overlap = final_ids.intersection(ordinary_test_ids)
    if overlap:
        raise V3PlanIntegrityError("ordinary test membership overlaps final holdout")


def build_v3_gate_b_plan(
    connection: Connection,
    *,
    as_of: datetime,
    ...
    config_sha256 = canonical_hash(_config_payload(config))
