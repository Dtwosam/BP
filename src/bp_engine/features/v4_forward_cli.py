...
from bp_engine.features.v4_forward import V4ForwardCycleStats, run_v4_forward_cycle
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one research-only outcome-blind V4 forward coverage cycle"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    once = subparsers.add_parser("once", help="run one bounded V4 forward coverage cycle")
...
def require_research_zero_money(settings: Settings) -> None:
    ...
    if not safe:
        raise ValueError(
            "V4 forward coverage requires RESEARCH/live-disabled/zero-money safety"
        )
...
def _run(args: argparse.Namespace) -> V4ForwardCycleStats:
...
def _payload(stats: V4ForwardCycleStats) -> dict[str, object]:
    payload = asdict(stats)
    payload["cycle_at"] = stats.cycle_at.isoformat()
    payload["epoch"] = stats.epoch.isoformat()
    return payload
