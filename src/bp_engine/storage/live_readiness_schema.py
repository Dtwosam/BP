...
live_readiness_checks = Table(
    ...
    CheckConstraint("char_length(candidate_git_sha) = 64", name="ck_live_readiness_candidate_sha"),
    CheckConstraint("char_length(semantic_sha256) = 64", name="ck_live_readiness_semantic_sha"),
...
live_risk_decisions = Table(
    ...
    Column("semantic_sha256", String(64), nullable=False),
    Column("created_at", DateTime(timezone=True), nullable=False),
    CheckConstraint("char_length(prediction_semantic_sha256) = 64", name="ck_live_risk_prediction_sha"),
    CheckConstraint("char_length(policy_sha256) = 64", name="ck_live_risk_policy_sha"),
    CheckConstraint("char_length(semantic_sha256) = 64", name="ck_live_risk_semantic_sha"),
...
live_order_intents = Table(
    ...
    CheckConstraint("limit_price > 0 AND limit_price <= 1", name="ck_live_intent_limit_price"),
    CheckConstraint("char_length(semantic_sha256) = 64", name="ck_live_intent_semantic_sha"),
    UniqueConstraint("intent_id", name="uq_live_intent_id"),
    UniqueConstraint("prediction_id", "policy_version", name="uq_live_intent_prediction_policy"),
)
...
live_order_events = Table(
    ...
    Column("semantic_sha256", String(64), nullable=False),
    Column("created_at", DateTime(timezone=True), nullable=False),
    CheckConstraint("char_length(semantic_sha256) = 64", name="ck_live_event_semantic_sha"),
    UniqueConstraint("event_key", name="uq_live_event_key"),
)
Index("ix_live_event_intent_time", live_order_events.c.intent_id, live_order_events.c.observed_at)
...
live_reconciliation_runs = Table(
    ...
    Column("semantic_sha256", String(64), nullable=False),
    Column("created_at", DateTime(timezone=True), nullable=False),
    CheckConstraint("unresolved_count >= 0", name="ck_live_reconciliation_unresolved_nonnegative"),
    CheckConstraint("critical_count >= 0 AND critical_count <= unresolved_count", name="ck_live_reconciliation_critical_count"),
    CheckConstraint("char_length(semantic_sha256) = 64", name="ck_live_reconciliation_semantic_sha"),
