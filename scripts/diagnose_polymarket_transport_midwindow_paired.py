...
def _five_window(now: datetime) -> tuple[datetime, datetime, datetime]:
    epoch = int(now.timestamp())
    start_epoch = (epoch // 300) * 300
    start = datetime.fromtimestamp(start_epoch, tz=UTC)
    return start - timedelta(seconds=300), start, start + timedelta(seconds=300)


def _target_action(now: datetime) -> datetime:
    _, current_start, next_start = _five_window(now)
    action = current_start + timedelta(seconds=ACTION_OFFSET_SECONDS)
    if (action - now).total_seconds() < MIN_ACTION_LEAD_SECONDS:
        action = next_start + timedelta(seconds=ACTION_OFFSET_SECONDS)
    return action
...
async def main() -> None:
    ...
    current_start = action_at - timedelta(seconds=ACTION_OFFSET_SECONDS)
    future_start = current_start + timedelta(seconds=300)
    ...
    active_five = _find_market(markets, 300, current_start)
    future_five = _find_market(markets, 300, future_start)
    ...
    future_fifteen = [
        ...
        and market.active
        and market.window_start_at > action_at
    ]
    if not future_fifteen:
        raise RuntimeError("no future 15m baseline market available")
    anchor = max(future_fifteen, key=lambda market: market.window_start_at)
    baseline_assets = _assets(anchor)
