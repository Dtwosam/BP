...
PROJECT="${PHASE9_PROJECT:-project-4397f2c0-7098-4c1c-abb}"
ZONE="${PHASE9_ZONE:-us-east1-c}"
VM="${PHASE9_VM:-bp-recorder}"
BRANCH="${PHASE9_BRANCH:-build/phase-9-probability-calibration-edge}"
EXPECTED_HEAD="${PHASE9_HEAD:-}"

if [[ -z "$EXPECTED_HEAD" ]]; then
  echo "PHASE9_HEAD must be set to the verified candidate SHA" >&2
  exit 2
fi
...
echo "VM=$VM"
echo "ZONE=$ZONE"
echo "PHASE9_HEAD=$EXPECTED_HEAD"

REMOTE_SCRIPT=$(cat <<REMOTE
set -Eeuo pipefail
SHA='$EXPECTED_HEAD'
BRANCH='$BRANCH'
WT="/var/tmp/bp-phase9-wt-\${SHA:0:12}-\$\$"
SRC="/var/tmp/bp-phase9-src-\${SHA:0:12}-\$\$"
LOG=/var/lib/bp/evidence/phase9-host-acceptance-latest.log

git -C /opt/bp fetch --no-tags origin \
  "refs/heads/\$BRANCH:refs/remotes/origin/\$BRANCH"
FETCHED=\$(git -C /opt/bp rev-parse "refs/remotes/origin/\$BRANCH")
if [[ "\$FETCHED" != "\$SHA" ]]; then
  echo "PHASE9_HOST_ACCEPTANCE=FAIL"
...
BP_REPO="\$SRC" BP_VERIFIED_HEAD="\$WORKTREE_HEAD" bash \
  "\$SRC/scripts/deploy/phase9_host_acceptance.sh" "\$SHA" \
  2>&1 | tee "\$LOG"
RC=\${PIPESTATUS[0]}
set -e
...
echo "Connecting to $VM for Phase 9 host acceptance..."
