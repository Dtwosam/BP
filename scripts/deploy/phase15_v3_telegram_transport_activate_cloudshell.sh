...
bash "$ROOT/scripts/deploy/phase15_v3_telegram_approval_status_cloudshell.sh"   >/tmp/bp-phase15-telegram-listener-status.txt ||
  fail "telegram_listener_status_not_pass"
grep -q '^LISTENER_BINDING_CURRENT=true
...
echo "WAITING_FOR_FRESH_TELEGRAM_APPROVAL=true"
   /tmp/bp-phase15-telegram-listener-status.txt ||
  fail "telegram_listener_not_exact_current_main"
grep -q '^HANDOFF_CONFIGURED=false
EXPECTED_EXECUTOR_SHA256=$(python3 - "$ROOT/scripts/deploy/phase15_v3_canary_executor.py" <<'PY'
import hashlib
import sys
from pathlib import Path
...
grep -q '^TELEGRAM_TRANSPORT_STAGE_READY=true
EXPECTED_EXECUTOR_SHA256=$(python3 - "$ROOT/scripts/deploy/phase15_v3_canary_executor.py" <<'PY'
import hashlib
import sys
from pathlib import Path
...
echo "REAL_ORDER_SUBMITTED=false"
echo "WAITING_FOR_FRESH_TELEGRAM_APPROVAL=true"
   /tmp/bp-phase15-telegram-stage-status.txt ||
