...
PROJECT="${PHASE12_PROJECT:-project-4397f2c0-7098-4c1c-abb}"
ZONE="${PHASE12_ZONE:-us-east1-c}"
VM="${PHASE12_VM:-bp-recorder}"
BRANCH="${PHASE12_BRANCH:-phase-12-paper-execution-v1}"
EXPECTED_HEAD="${PHASE12_HEAD:-}"

if [[ -z "$EXPECTED_HEAD" ]]; then
  echo "PHASE12_HEAD must be set to the verified candidate SHA" >&2
  exit 2
fi
...
LOG=/var/lib/bp/evidence/phase12-host-acceptance-latest.log
UNIT="bp-phase12-host-acceptance-\$SHORT"
...
if systemctl show "\$UNIT_NAME" -p LoadState --value 2>/dev/null | grep -qv '^not-found$'; then
  ...
  if [[ "\$ACTIVE" == "activating" || "\$SUB" == "start" || "\$SUB" == "running" ]]; then
    echo "Existing Phase 12 acceptance job is still running; reattaching."
  elif grep -q '^PHASE12_HOST_ACCEPTANCE=PASS$' "\$LOG" 2>/dev/null; then
    show_result
    exit 0
  else
...
SUB=\$(systemctl show "\$UNIT_NAME" -p SubState --value 2>/dev/null || true)
if [[ "\$ACTIVE" != "activating" && "\$SUB" != "start" && "\$SUB" != "running" ]]; then
  WT="/var/tmp/bp-phase12-wt-\$SHORT-\$\$"
  SRC="\$RUNTIME_ROOT/bp-phase12-src-\$SHORT-\$\$"
  JOB="\$RUNTIME_ROOT/bp-phase12-host-acceptance-\$SHORT-\$\$.sh"
...
BP_REPO="\$SRC" BP_VERIFIED_HEAD="\$WORKTREE_HEAD" bash \\
  "\$SRC/scripts/deploy/phase12_host_acceptance.sh" "\$SHA" \\
  >"\$LOG" 2>&1
RC=\\\$?
set -e
if [[ "\\\$RC" -ne 0 ]]; then
  echo "PHASE12_WRAPPER_RC=\\\$RC" >>"\$LOG"
fi
exit "\\\$RC"
JOB
  chmod 700 "\$JOB"
  ...
  systemd-run \
    ...
    "\$JOB"
  echo "Started disconnect-resilient Phase 12 acceptance job \$UNIT_NAME."
fi
...
if ! grep -q '^PHASE12_HOST_ACCEPTANCE=PASS$' "\$LOG"; then
  echo "PHASE12_HOST_ACCEPTANCE=FAIL"
...
echo "Connecting to $VM for Phase 12 host acceptance..."
