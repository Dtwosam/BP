...
PROJECT="${PHASE8_PROJECT:-project-4397f2c0-7098-4c1c-abb}"
ZONE="${PHASE8_ZONE:-us-east1-c}"
VM="${PHASE8_VM:-bp-recorder}"
BRANCH="${PHASE8_BRANCH:-build/phase-8-walk-forward-backtester}"
EXPECTED_HEAD="${PHASE8_HEAD:-}"

if [[ -z "$EXPECTED_HEAD" ]]; then
  echo "PHASE8_HEAD must be set to the verified candidate SHA" >&2
  exit 2
fi
...
echo "VM=$VM"
echo "ZONE=$ZONE"
echo "PHASE8_HEAD=$EXPECTED_HEAD"

REMOTE_SCRIPT=$(cat <<REMOTE
set -Eeuo pipefail
SHA='$EXPECTED_HEAD'
BRANCH='$BRANCH'
WT="/var/tmp/bp-phase8-wt-\${SHA:0:12}-\$\$"
SRC="/var/tmp/bp-phase8-src-\${SHA:0:12}-\$\$"
LOG=/var/lib/bp/evidence/phase8-host-acceptance-latest.log

git -C /opt/bp fetch --no-tags origin \
  "refs/heads/\$BRANCH:refs/remotes/origin/\$BRANCH"
FETCHED=\$(git -C /opt/bp rev-parse "refs/remotes/origin/\$BRANCH")
if [[ "\$FETCHED" != "\$SHA" ]]; then
  echo "PHASE8_HOST_ACCEPTANCE=FAIL"
...
BP_REPO="\$SRC" BP_VERIFIED_HEAD="\$WORKTREE_HEAD" bash \
  "\$SRC/scripts/deploy/phase8_host_acceptance.sh" "\$SHA" \
  2>&1 | tee "\$LOG"
RC=\${PIPESTATUS[0]}
set -e
...
echo "PHASE8_HOST_ACCEPTANCE=PASS"
echo "LOG=\$LOG"
REMOTE
)
REMOTE_B64=$(printf '%s' "$REMOTE_SCRIPT" | base64 -w0)

echo "Connecting to $VM for Phase 8 host acceptance..."
