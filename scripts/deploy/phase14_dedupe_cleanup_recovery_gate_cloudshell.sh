...
ZONE='us-east1-c'
VM='bp-recorder'
HELPER_HEAD="${PHASE14_DEDUPE_CLEANUP_RECOVERY_HELPER_HEAD:-}"
APPROVAL="${PHASE14_DEDUPE_CLEANUP_RECOVERY_APPROVAL:-}"

FROM_HEAD='e9c7afc1536880e4612cb6e3d1a7282fa37c69f5'
CANDIDATE_BRANCH='ops/phase14-dedupe-cleanup-recovery-candidate'
CANDIDATE_HEAD='71b33d3beaba4a11ef93e7c5bde1c517323f3440'
...
RUNTIME_PATH='src/bp_engine/storage/maintenance.py'
TEST_PATH='tests/storage/test_partitioned_raw_postgres.py'

fail_local() {
  echo "PHASE14_DEDUPE_CLEANUP_RECOVERY_GATE=FAIL" >&2
  echo "REASON=$1" >&2
  exit 1
}
...
EXPECTED_APPROVAL="I_APPROVE_PHASE14_DEDUPE_CLEANUP_RECOVERY:${HELPER_HEAD}:${FROM_HEAD}:${CANDIDATE_HEAD}:${STORAGE_EVIDENCE_SHA256}"
...
HELPER_HEAD="${PHASE14_DEDUPE_CLEANUP_RECOVERY_HELPER_HEAD:?}"
FROM_HEAD="${PHASE14_DEDUPE_CLEANUP_RECOVERY_FROM_HEAD:?}"
CANDIDATE_BRANCH="${PHASE14_DEDUPE_CLEANUP_RECOVERY_CANDIDATE_BRANCH:?}"
CANDIDATE_HEAD="${PHASE14_DEDUPE_CLEANUP_RECOVERY_CANDIDATE_HEAD:?}"
ENV_FILE="${PHASE14_DEDUPE_CLEANUP_RECOVERY_ENV_FILE:?}"
STORAGE_EVIDENCE="${PHASE14_DEDUPE_CLEANUP_RECOVERY_STORAGE_EVIDENCE:?}"
STORAGE_EVIDENCE_SHA256="${PHASE14_DEDUPE_CLEANUP_RECOVERY_STORAGE_EVIDENCE_SHA256:?}"
RUNTIME_PATH="${PHASE14_DEDUPE_CLEANUP_RECOVERY_RUNTIME_PATH:?}"
TEST_PATH="${PHASE14_DEDUPE_CLEANUP_RECOVERY_TEST_PATH:?}"
EXPECTED_RUNTIME_BLOB="${PHASE14_DEDUPE_CLEANUP_RECOVERY_RUNTIME_BLOB:?}"
EXPECTED_TEST_BLOB="${PHASE14_DEDUPE_CLEANUP_RECOVERY_TEST_BLOB:?}"
...
run_soak_gate() {
  SOAK_FILE=$(mktemp /var/tmp/bp-phase14-dedupe-cleanup-recovery-soak.XXXXXX.json)
...
verify_dashboard_safety() {
  SNAPSHOT_FILE=$(mktemp /var/tmp/bp-phase14-dedupe-cleanup-recovery-snapshot.XXXXXX.json)
...
rollback() {
  set +e
  echo "PHASE14_DEDUPE_CLEANUP_RECOVERY_ROLLBACK=START" >&2
  ...
  echo "PHASE14_DEDUPE_CLEANUP_RECOVERY_ROLLBACK=COMPLETE" >&2
  set -e
}
...
DISK_BEFORE=$(mktemp /var/tmp/bp-phase14-dedupe-cleanup-recovery-disk-before.XXXXXX.json)
...
DISK_AFTER_PRESTART=$(mktemp /var/tmp/bp-phase14-dedupe-cleanup-recovery-disk-prestart.XXXXXX.json)
...
DISK_AFTER_ACTIVE=$(mktemp /var/tmp/bp-phase14-dedupe-cleanup-recovery-disk-after.XXXXXX.json)
...
EVIDENCE_TMP=$(mktemp /var/tmp/bp-phase14-dedupe-cleanup-recovery-evidence.XXXXXX.json)
...
EVIDENCE_PATH="$EVIDENCE_DIR/phase14-dedupe-cleanup-recovery-$STAMP.json"
install -o bp -g bp -m 0640 "$EVIDENCE_TMP" "$EVIDENCE_PATH"
ROLLBACK_ARMED=0

cat "$EVIDENCE_PATH"
echo "EVIDENCE_FILE=$EVIDENCE_PATH"
echo "PHASE14_DEDUPE_CLEANUP_RECOVERY_GATE=PASS"
REMOTE
)
...
gcloud compute ssh "$VM"   --project="$PROJECT"   --zone="$ZONE"   --command="printf '%s' '$REMOTE_B64' | base64 -d | sudo env PHASE14_DEDUPE_CLEANUP_RECOVERY_HELPER_HEAD=$HELPER_HEAD_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_FROM_HEAD=$FROM_HEAD_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_CANDIDATE_BRANCH=$CANDIDATE_BRANCH_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_CANDIDATE_HEAD=$CANDIDATE_HEAD_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_ENV_FILE=$ENV_FILE_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_STORAGE_EVIDENCE=$STORAGE_EVIDENCE_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_STORAGE_EVIDENCE_SHA256=$STORAGE_EVIDENCE_SHA256_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_RUNTIME_PATH=$RUNTIME_PATH_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_TEST_PATH=$TEST_PATH_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_RUNTIME_BLOB=$RUNTIME_BLOB_Q PHASE14_DEDUPE_CLEANUP_RECOVERY_TEST_BLOB=$TEST_BLOB_Q bash"
