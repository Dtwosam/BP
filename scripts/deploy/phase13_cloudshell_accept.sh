...
PROJECT="${PHASE13_PROJECT:-project-4397f2c0-7098-4c1c-abb}"
ZONE="${PHASE13_ZONE:-us-east1-c}"
VM="${PHASE13_VM:-bp-recorder}"
BRANCH="${PHASE13_BRANCH:-phase-13-improvement-loop-v1}"
EXPECTED_HEAD="${PHASE13_HEAD:-}"
ENV_FILE="${PHASE13_ENV_FILE:-/etc/bp/bp.env}"

if [[ -z "$EXPECTED_HEAD" ]]; then
  echo "PHASE13_HEAD must be set to the verified candidate SHA" >&2
  exit 2
fi
...
echo "VM=$VM"
echo "ZONE=$ZONE"
echo "PHASE13_HEAD=$EXPECTED_HEAD"
...
LOG=/var/lib/bp/evidence/phase13-host-acceptance-latest.log
UNIT="bp-phase13-host-acceptance-\$SHORT"
...
if systemctl show "\$UNIT_NAME" -p LoadState --value 2>/dev/null | grep -qv '^not-found$'; then
  ...
  if [[ "\$ACTIVE" == "activating" || "\$SUB" == "start" || "\$SUB" == "running" ]]; then
    echo "Existing Phase 13 acceptance job is still running; reattaching."
  elif grep -q '^PHASE13_HOST_ACCEPTANCE=PASS$' "\$LOG" 2>/dev/null; then
    show_result
    exit 0
  else
...
SUB=\$(systemctl show "\$UNIT_NAME" -p SubState --value 2>/dev/null || true)
if [[ "\$ACTIVE" != "activating" && "\$SUB" != "start" && "\$SUB" != "running" ]]; then
  WT="/var/tmp/bp-phase13-wt-\$SHORT-\$\$"
  SRC="\$RUNTIME_ROOT/bp-phase13-src-\$SHORT-\$\$"
  JOB="\$RUNTIME_ROOT/bp-phase13-host-acceptance-\$SHORT-\$\$.sh"

  git -C /opt/bp fetch --no-tags origin \
    "refs/heads/\$BRANCH:refs/remotes/origin/\$BRANCH"
  FETCHED=\$(git -C /opt/bp rev-parse "refs/remotes/origin/\$BRANCH")
  if [[ "\$FETCHED" != "\$SHA" ]]; then
    echo "PHASE13_HOST_ACCEPTANCE=FAIL"
...
trap cleanup EXIT
set +e
BP_REPO="\$SRC" BP_VERIFIED_HEAD="\$WORKTREE_HEAD" BP_ENV_FILE="\$ENV_FILE" bash \\
  "\$SRC/scripts/deploy/phase13_host_acceptance.sh" "\$SHA" \\
  >"\$LOG" 2>&1
RC=\\\$?
set -e
if [[ "\\\$RC" -ne 0 ]]; then
  echo "PHASE13_WRAPPER_RC=\\\$RC" >>"\$LOG"
fi
exit "\\\$RC"
JOB
  chmod 700 "\$JOB"
  install -d /var/lib/bp/evidence
  : > "\$LOG"
  ...
  systemd-run \
    ...
    "\$JOB"
  echo "Started disconnect-resilient Phase 13 acceptance job \$UNIT_NAME."
fi
...
if [[ "\$RC" != "0" ]]; then
  echo "--- UNIT JOURNAL ---"
  journalctl -u "\$UNIT_NAME" -n 100 --no-pager || true
  exit "\$RC"
fi
if ! grep -q '^PHASE13_HOST_ACCEPTANCE=PASS$' "\$LOG"; then
  echo "PHASE13_HOST_ACCEPTANCE=FAIL"
...
echo "Connecting to $VM for Phase 13 host acceptance..."
