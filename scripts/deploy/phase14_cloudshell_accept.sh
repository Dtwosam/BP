...
PROJECT="${PHASE14_PROJECT:-project-4397f2c0-7098-4c1c-abb}"
ZONE="${PHASE14_ZONE:-us-east1-c}"
VM="${PHASE14_VM:-bp-recorder}"
BRANCH="${PHASE14_BRANCH:-phase-14-live-readiness-v1}"
EXPECTED_HEAD="${PHASE14_HEAD:-}"
ENV_FILE="${PHASE14_ENV_FILE:-/etc/bp/bp.env}"

if [[ -z "$EXPECTED_HEAD" ]]; then
  echo "PHASE14_HEAD must be set to the verified candidate SHA" >&2
  exit 2
fi
...
echo "VM=$VM"
echo "ZONE=$ZONE"
echo "PHASE14_HEAD=$EXPECTED_HEAD"
...
LOG=/var/lib/bp/evidence/phase14-host-acceptance-latest.log
UNIT="bp-phase14-host-acceptance-\$SHORT"
...
if systemctl show "\$UNIT_NAME" -p LoadState --value 2>/dev/null | grep -qv '^not-found$'; then
  ...
  if [[ "\$ACTIVE" == "activating" || "\$SUB" == "start" || "\$SUB" == "running" ]]; then
    echo "Existing Phase 14 acceptance job is still running; reattaching."
  elif grep -q '^PHASE14_HOST_ACCEPTANCE=PASS$' "\$LOG" 2>/dev/null; then
    show_result
    exit 0
  else
...
SUB=\$(systemctl show "\$UNIT_NAME" -p SubState --value 2>/dev/null || true)
if [[ "\$ACTIVE" != "activating" && "\$SUB" != "start" && "\$SUB" != "running" ]]; then
  WT="/var/tmp/bp-phase14-wt-\$SHORT-\$\$"
  SRC="\$RUNTIME_ROOT/bp-phase14-src-\$SHORT-\$\$"
  JOB="\$RUNTIME_ROOT/bp-phase14-host-acceptance-\$SHORT-\$\$.sh"

  git -C /opt/bp fetch --no-tags origin \
    "refs/heads/\$BRANCH:refs/remotes/origin/\$BRANCH"
  FETCHED=\$(git -C /opt/bp rev-parse "refs/remotes/origin/\$BRANCH")
  if [[ "\$FETCHED" != "\$SHA" ]]; then
    echo "PHASE14_HOST_ACCEPTANCE=FAIL"
...
BP_REPO="\$SRC" BP_VERIFIED_HEAD="\$WORKTREE_HEAD" BP_ENV_FILE="\$ENV_FILE" bash \\
  "\$SRC/scripts/deploy/phase14_host_acceptance.sh" "\$SHA" \\
  >"\$LOG" 2>&1
RC=\\\$?
set -e
if [[ "\\\$RC" -ne 0 ]]; then
  echo "PHASE14_WRAPPER_RC=\\\$RC" >>"\$LOG"
fi
exit "\\\$RC"
JOB
  chmod 700 "\$JOB"
  ...
  systemd-run \
    ...
    "\$JOB"
  echo "Started disconnect-resilient Phase 14 acceptance job \$UNIT_NAME."
fi
...
if ! grep -q '^PHASE14_HOST_ACCEPTANCE=PASS$' "\$LOG"; then
  echo "PHASE14_HOST_ACCEPTANCE=FAIL"
...
echo "Connecting to $VM for Phase 14 host acceptance..."
