...
PROJECT="${PHASE11_PROJECT:-project-4397f2c0-7098-4c1c-abb}"
ZONE="${PHASE11_ZONE:-us-east1-c}"
VM="${PHASE11_VM:-bp-recorder}"
BRANCH="${PHASE11_BRANCH:-phase-11-dashboard-v1}"
EXPECTED_HEAD="${PHASE11_HEAD:-}"

if [[ -z "$EXPECTED_HEAD" ]]; then
  echo "PHASE11_HEAD must be set to the verified candidate SHA" >&2
  exit 2
fi
...
echo "VM=$VM"
echo "ZONE=$ZONE"
echo "PHASE11_HEAD=$EXPECTED_HEAD"
...
LOG=/var/lib/bp/evidence/phase11-host-acceptance-latest.log
UNIT="bp-phase11-host-acceptance-\$SHORT"
...
if systemctl show "\$UNIT_NAME" -p LoadState --value 2>/dev/null | grep -qv '^not-found$'; then
  ...
  if [[ "\$ACTIVE" == "activating" || "\$SUB" == "start" || "\$SUB" == "running" ]]; then
    echo "Existing Phase 11 acceptance job is still running; reattaching."
  elif grep -q '^PHASE11_HOST_ACCEPTANCE=PASS$' "\$LOG" 2>/dev/null; then
    show_result
    exit 0
  else
...
SUB=\$(systemctl show "\$UNIT_NAME" -p SubState --value 2>/dev/null || true)
if [[ "\$ACTIVE" != "activating" && "\$SUB" != "start" && "\$SUB" != "running" ]]; then
  WT="/var/tmp/bp-phase11-wt-\$SHORT-\$\$"
  SRC="\$RUNTIME_ROOT/bp-phase11-src-\$SHORT-\$\$"
  JOB="\$RUNTIME_ROOT/bp-phase11-host-acceptance-\$SHORT-\$\$.sh"

  git -C /opt/bp fetch --no-tags origin \
    "refs/heads/\$BRANCH:refs/remotes/origin/\$BRANCH"
  FETCHED=\$(git -C /opt/bp rev-parse "refs/remotes/origin/\$BRANCH")
  if [[ "\$FETCHED" != "\$SHA" ]]; then
    echo "PHASE11_HOST_ACCEPTANCE=FAIL"
...
BP_REPO="\$SRC" BP_VERIFIED_HEAD="\$WORKTREE_HEAD" bash \\
  "\$SRC/scripts/deploy/phase11_host_acceptance.sh" "\$SHA" \\
  >"\$LOG" 2>&1
RC=\\\$?
set -e
if [[ "\\\$RC" -ne 0 ]]; then
  echo "PHASE11_WRAPPER_RC=\\\$RC" >>"\$LOG"
fi
exit "\\\$RC"
JOB
  chmod 700 "\$JOB"
  ...
  systemd-run \
    ...
    "\$JOB"
  echo "Started disconnect-resilient Phase 11 acceptance job \$UNIT_NAME."
fi
...
if ! grep -q '^PHASE11_HOST_ACCEPTANCE=PASS$' "\$LOG"; then
  echo "PHASE11_HOST_ACCEPTANCE=FAIL"
...
echo "Connecting to $VM for Phase 11 host acceptance..."
