...
def _next_five_only_boundary(now: datetime) -> datetime:
    epoch = int(now.timestamp())
    candidate = ((epoch // 300) + 1) * 300
    while True:
        lead = candidate - now.timestamp()
        is_fifteen_boundary = candidate % 900 == 0
        if lead >= MIN_BOUNDARY_LEAD_SECONDS and not is_fifteen_boundary:
            return datetime.fromtimestamp(candidate, tz=UTC)
        candidate += 300


def _next_fifteen_start(after: datetime) -> datetime:
    epoch = int(after.timestamp())
    start = ((epoch // 900) + 1) * 900
...
async def main() -> None:
    ...
    boundary = _next_five_only_boundary(started_at)
    target_start = _next_fifteen_start(boundary)
    anchor_start = target_start + timedelta(seconds=900)
    ...
    target = _find_market(markets, 900, target_start)
    anchor = _find_market(markets, 900, anchor_start)
        ...
        json.dumps(
            {
                ...
                "boundary_is_fifteen_minute": int(boundary.timestamp()) % 900 == 0,
                ...
                "target_active_during_observation": target.window_start_at.timestamp() <= stop_at,
