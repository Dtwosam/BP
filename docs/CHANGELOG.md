...
- Added a V3-only prospective signal runtime that verifies the exact frozen model SHA, rebuilds the original BTC-native 240-second feature input, applies the frozen calibration/model, and uses Polymarket compact books only for the frozen executable-edge decision.
