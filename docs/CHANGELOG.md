...
- Engineering hardening for the Phase 15 canary timing window: frozen V3 still predicts at 240s in a 300s market (maximum theoretical 60s to expiry), while avoidable polling latency is reduced by changing the frozen-V3 paper-executor poll from 5s to 1s and the prepare-only watcher default from 2s to 0.5s. Prepared outputs now include scheduled/predicted/paper-order/prepare timing diagnostics. Model, offset, edge, $5 target, live risk, arm, and submission semantics are unchanged. This is not production-deployed and requires separate rollout authorization.
