...
### 4.3.5 Telegram transport recovery-required, not activated — 26 September 2026

The previous inactive transport stage was rolled back after an earlier activation attempt failed closed with `REASON=executor_transport_service_activation_failed`. That earlier attempt submitted no real order, left global and Phase-15 live trading disabled, and re-engaged the Johannesburg kill switch. PRs #298 and #299 corrected the first observed staging/runtime and recovery-preflight defects without changing wallet, arming, live-trading, IAM-scope, or order-submission semantics.

A corrected deterministic release was subsequently staged on both production hosts at release head `def15988b87fe90436fa9ee1458e640ba67bb1af`, release SHA-256 `6b35d1ce72e581dbe75531170cf9b078ed71e23416431ea8f0f965822a8d9d24`, and stage ID `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6`. Stage status and Pub/Sub readiness passed with zero blockers before activation retry.

The second activation attempt failed closed at executor transport service startup. Post-failure diagnostics verified the Johannesburg executor side: all four transport services were inactive/disabled, all six executor runtime env/key files were absent, the kill switch was engaged, the official account remained clean with zero open orders, `activation_valid=false`, `submission_ready=false`, and `live_order_submitted=false`. Recorder-side cleanup was not part of that diagnostic and must be independently revalidated by the guarded rollback preflight. The runtime venv was usable by `bp-transport`, proving the earlier venv fix remained effective. Durable sanitized evidence is `docs/evidence/phase-15-v3-telegram-transport-second-activation-failure-20260926.json`.

The remaining defects are filesystem-boundary defects, not trading-logic defects. Under the stage installer's `umask 077`, the file-only transport archive extraction left intermediate release directories inaccessible to `bp-transport`, so the receiver and claim workers could execute Python but could not open their staged scripts. Separately, transport queues were staged beneath root-only `/var/lib/bp-canary`; the unprivileged transport service could not traverse that parent, and the capability-stripped execution-authorization service could not read the `bp-transport`-private ready queue.

The reviewed recovery design therefore keeps privileged canary/wallet state isolated while separating the unprivileged carrier state: transport inbox/claim/ready state moves to `/var/lib/bp-telegram-transport`; only the dedicated `bp-transport` group receives required traversal/read access; the execution-authorization service remains root with no added capabilities and gains only group-read access to immutable ready bundles; privileged authorization/handoff state remains under root-only `/var/lib/bp-canary`; and extracted runtime files/directories receive explicit service-readable ownership/modes. No model, prediction, sizing, risk, wallet, arm, or order-submission logic is changed.

Stage `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6` is now recovery-required and must not be activated again. The required sequence is rollback of that inactive stage, corrected restage, independent read-only stage/Pub/Sub/readiness verification, source-truth update for the new stage, and only then another fail-closed activation attempt. The existing transport activation authorization remains in force but does not authorize any real order. Exactly one fresh private Telegram `APPROVE` for a new exact frozen-V3 $5 intent/request remains mandatory after successful activation before the second canary can invoke the executor, and official reconciliation remains mandatory before any third live action.
