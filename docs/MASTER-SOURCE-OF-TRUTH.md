...
Under explicit rollback-and-restage-only authorization, inactive stage `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6` was removed from both production hosts. The guarded rollback returned PASS on recorder and executor with transport services inactive/disabled, no IAM or Pub/Sub resource mutation, global live trading disabled, and no real order submitted.

The corrected deterministic release was then staged successfully on both production hosts at exact release head `ba0bc324cab50bf0809c8eed4e019c13bc4c6653`, release SHA-256 `ed6ee3ccfe463c71325d5e8ef6e525c3cbb640b29791ca989e91927938ca4b69`, manifest SHA-256 `98745099fbc40f036736d39677133b3b931ff1f157b0d83b605908d194553080`, and stage ID `phase15-telegram-stage-05b83214b159772872bc7347`. Publisher and executor staging returned PASS; all transport services remain inactive/disabled, runtime environment/key material is absent, and Johannesburg safe-idle was preserved.
...
Production listener repair completed PASS on 26 September 2026 using the separately authorized listener-only helper. The repaired listener is deployed at exact head `72aea2868cae1991af5bc66fab75e2ee518e0db7`; the service is active+enabled with a stable PID, the exact `bp_engine.execution.telegram_approval` import succeeds as user `bp`, the existing root-owned three-key Telegram env was reused in place, the listener binding is current, handoff remains absent, the last-15-minute error count is zero, and recorder/predictor/paper core service PIDs were preserved. Global live trading remained disabled and no real order was submitted. The one-shot listener-repair authorization is now consumed: source truth records `RUNTIME_REPAIR_PASS`, `runtime_repair_required=false`, and `runtime_repair_authorized=false`. Historical restart-loop evidence remains preserved under `previous_*` fields. Durable repair evidence is `docs/evidence/phase-15-v3-telegram-approval-listener-runtime-repair-production-20260926.json`.

At the post-listener-repair checkpoint, the then-existing transport activation authorization remained `AUTHORIZED_NOT_ACTIVATED`. A fresh read-only activation-readiness check after repair returned `activation_ready=true` with zero blockers and Johannesburg safe-idle preserved. That authorization was later superseded for future retries by the helper-artifact change described below; Telegram `APPROVE`, executor arming, and order submission remain separate later boundaries.

A subsequent authorized activation attempt from exact main `837ba96aae74addd97892b9330c86f270e529a53` passed the read-only readiness gate, crossed the transport mutation boundary, updated the resource-scoped Pub/Sub IAM bindings, and created enablement symlinks for the four Johannesburg transport services, but then failed closed at `executor_transport_service_activation_failed`. Read-only post-failure revalidation proved fail-closed cleanup completed on both hosts: all four Johannesburg transport services are inactive+disabled, executor transport env/key material is absent, the kill switch is engaged, executor health is safe-idle with zero open orders and no live order, the recorder publisher is inactive+disabled, recorder transport material is absent, and the approval listener is active+enabled. Unit journals identify the root cause: the activation helper wrote 32 raw random bytes to both key files, while the transport and origin runtime loaders require one UTF-8 base64url value decoding to exactly 32 bytes. Receiver and claim therefore died with `UnicodeDecodeError` before any Telegram approval, executor arm, or order submission.

The candidate helper fix encodes the random key bytes as one-line base64url text and validates the decoded length before mutation. It also replaces the nested escaped `printf` used by fail-closed cleanup with `echo`, correcting the observed `activation-failure-safe-stopn` marker while preserving file-presence kill semantics. The staged runtime itself remains read-only healthy and does not require restage. However, the prior activation authorization was bound to helper blob `d2560354d10c9964ba0a5a0161f1be9ffb2dfd86`; changing the helper invalidates that exact-artifact authorization for any retry. Source truth first recorded stage readiness restored and `REAUTHORIZATION_REQUIRED` for candidate helper blob `83157c6c04b9a996bab51012b4bda4dd31062320`. After PR #310 merged as current main `23372ef1b9038a756a8187209873de363498aae0` with post-merge CI green, the user explicitly reauthorized transport activation for exactly stage `phase15-telegram-stage-05b83214b159772872bc7347` using that exact repaired helper blob. That exact reauthorization was then exercised successfully from main `469049a9f6361f9c656e3d176029b10db7359689`: listener, stage, Pub/Sub, and activation-readiness gates all passed first with zero blockers; the activation helper returned `PHASE15_V3_TELEGRAM_TRANSPORT_ACTIVATE=PASS`; the existing topic/subscription were reused; publisher plus all four Johannesburg transport services became active; global live trading remained disabled; the executor remained safe-idle; and no real order was submitted. The activation authorization is now consumed, and any activation retry requires fresh explicit authorization. Source truth records `ACTIVATED_WAITING_FOR_FRESH_TELEGRAM_APPROVAL`, but there is still no pending intent: the next safe action is to prepare and review one fresh NEW frozen-V3 paper-derived $5 intent before any private Telegram `APPROVE`. Executor invocation and order submission remain unperformed, and official reconciliation remains mandatory after any future one-shot execution. Durable failure evidence is `docs/evidence/phase-15-v3-telegram-transport-third-activation-failure-20260926.json`; the repaired-helper authorization is `docs/evidence/phase-15-v3-telegram-transport-activation-reauthorization-20260926.json`; successful activation evidence is `docs/evidence/phase-15-v3-telegram-transport-activation-pass-production-20260926.json`. The already-authorized prepare-only watcher was then started successfully from exact main `c2034bee883786c5e31558cb5717d98780baa8c3` as run `phase15-prepare-watch-20260926T144337Z-c2034bee`, but it stopped safely about 40 seconds later before preparing any intent. Read-only follow/status reported `canary_submission_attempt_limit_reached` with `submission_attempt_count=1`, `submission_attempt_consumed=false`, service inactive, `NO_REAL_ORDER_SUBMITTED=true`, and no Telegram approval, executor arm/invocation, or order submission. The root cause is a legacy preparation-layer lifetime ledger ceiling: generic `v3-live-canary-v1` preparation still counted the officially reconciled first canary against its original one-attempt/one-accepted-order cap, even though source truth separately authorizes exactly one second canary. The repository correction keeps legacy/default callers at one lifetime attempt/order, while only the source-truth-gated second-canary prepare watcher passes an authorized lifetime ledger ceiling of two (reconciled first canary plus one second canary); actual second-canary execution remains independently one-shot through the root-owned `/var/lib/bp-canary/telegram-live-handoff/second-canary.attempt.json` marker. Because this correction changes bound prepare runtime files, the watcher required fresh explicit production authorization for the exact corrected release. That authorization is now recorded for main `51e4cf1fa5063ff0f4a263dd5ec1646dc18684cf`, binding start-helper blob `0a98b03e35d8019a85063e45d1a979fea96c0532`, runner blob `efe6dc9c3b37f5788b701107366bb634a4b1f357`, service-unit blob `5e20c65edd57e398d2106c7f6fe93fbb477b7572`, and canary blob `df5e60b1b2ba632fd77d65103509fac70187be7d`. That corrected restart was then exercised successfully from exact main `f3de8c008d6a50584dc71a172ca4dcb4275f73d8` as run `phase15-prepare-watch-20260926T155010Z-f3de8c00`. All four authorized blobs matched, the watcher service became active, `NO_REAL_ORDER_SUBMITTED=true`, `ARM_AUTOMATED=false`, and `SUBMISSION_AUTOMATED=false`. The restart authorization is consumed. There is still no pending intent, and Telegram `APPROVE`, executor arm/invocation, and order submission remain separate and unperformed. The only next action is read-only status/follow observation of this exact corrected run until one fresh NEW frozen-V3 $5 candidate appears or the run terminates. Evidence: `docs/evidence/phase-15-v3-second-telegram-prepare-watch-start-production-20260926.json`, `docs/evidence/phase-15-v3-second-telegram-prepare-watch-ledger-stop-production-20260926.json`, and `docs/evidence/phase-15-v3-corrected-second-telegram-prepare-watch-restart-pass-production-20260926.json`.
...
The one authorized network submission attempt is consumed. No retry or second order is authorized. The accepted response plus successful cancellation did not by itself establish whether zero, partial, or full fill occurred before cancellation. The subsequent authenticated read-only official fill probe completed with two stable snapshots three seconds apart: zero open orders, no matching account trades for the exact external order ID, zero observed/confirmed matched shares, zero matched notional, and `fill_state=zero_fill_observed`. The canary therefore reconciles to zero live exposure and zero realized trade P&L. Submission and two-second cancellation behavior are validated; fill-price, slippage, and fill-quality behavior remain unobserved because no fill occurred. No additional live canary, automation, or sizing change is authorized by this result. Submission evidence: `docs/evidence/phase-15-v3-first-live-canary-submission-20260924.json`. Reconciliation evidence: `docs/evidence/phase-15-v3-first-live-canary-zero-fill-reconciliation-20260924.json`.
...
### Phase 15 first-canary DB reconciliation gap — 26 September 2026

A read-only production audit after corrected watcher run `phase15-prepare-watch-20260926T155010Z-f3de8c00` found three fresh frozen-V3 paper orders created after watcher activation. All three were `trade=true`, `executable=true`, and all three live-canary risk decisions failed only with `reconciliation_blocked`.

The first real canary remains externally verified as zero fill, but that external probe was intentionally read-only. The database ledger used by the live-risk engine has no post-submission reconciliation row after first-canary intent `live-intent-6cdfcfd28d0eb52f1ee0762bfd351409` was accepted and cancelled at `2026-09-24T17:20:14.900358Z`. Its latest `live_reconciliation_runs` row is the earlier pre-submission close `live-reconciliation-8da1de9a5375b842c1476e588201e18a` at `2026-09-24T17:04:50.389631Z`. Because a real submission attempt exists after that ledger row, the account snapshot correctly fails closed with unresolved critical reconciliation and blocks later candidates.

Engineering repair is ready in `scripts/deploy/phase15_v3_first_canary_db_reconciliation_cloudshell.sh`, candidate blob `b21456d222f8256b381d43d70c1373bbbee6d78d`. The helper freshly re-verifies the exact first canary as zero-fill from Johannesburg, requires safe-idle/zero-open-order account state, and idempotently writes only one `post_submission_official_zero_fill` reconciliation row with zero unresolved/critical count and a zero-exposure account snapshot. It does not approve Telegram, arm/invoke the executor, or submit an order. **Production execution of this DB repair is not authorized yet and requires fresh explicit authorization.** Evidence: `docs/evidence/phase-15-v3-first-canary-db-reconciliation-gap-readonly-20260926.json`.
