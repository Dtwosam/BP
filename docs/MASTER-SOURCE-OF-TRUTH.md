...
The Telegram approval listener remains installed and healthy on `bp-recorder` with handoff disabled. Transport activation remains separately authorized but not activated. Activation itself must submit no order and must leave the Johannesburg executor safe-idle with the kill switch engaged, no valid short-lived canary activation, submission not ready, and no live order reported. Exactly one fresh private Telegram `APPROVE` for a new exact frozen-V3 $5 intent/request remains mandatory before the second canary can invoke the executor, and official reconciliation remains mandatory before any third live action.
