...
Production listener repair is a separate mutation from transport activation. The user has now explicitly authorized that repair on `bp-recorder` from exact reviewed main `a60ff465ab307a360fb20fcefa5158f8258e46a8`, bound to repair-helper blob `028c5e0f747f388d8471b3be49ba288b9bbbecc4`. Source truth keeps the runtime condition as `RUNTIME_REPAIR_REQUIRED` and records authorization separately as `runtime_repair_authorized=true`. The dedicated repair helper may reuse the existing root-owned Telegram env in place and restart only the listener; it may not configure handoff, touch the executor, arm live trading, invoke Telegram `APPROVE`, activate the transport, or submit an order. The existing transport activation authorization remains `AUTHORIZED_NOT_ACTIVATED`, but the activation helper remains fail-closed on listener status until this repair is completed and independently verified. Durable failure evidence is `docs/evidence/phase-15-v3-telegram-approval-listener-runtime-failure-20260926.json`; repair-authorization evidence is `docs/evidence/phase-15-v3-telegram-approval-runtime-repair-authorization-20260926.json`.
