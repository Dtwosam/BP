...
This correction is **not deployed to production**. It changes `src/bp_engine/execution/canary.py`, which is a persistent-watcher binding path. Any production rollout/restart onto this corrected behavior requires a separate explicit production authorization and must preserve no-order prepare-only behavior.
