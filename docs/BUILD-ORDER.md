...
PR #84 closed the verified-target-binding integration record on `main` as `97bbe426182cfacaace2b8aa64d929059d34f606`. Final head `c9506d161613dce47fb8a71bd4bb8a87bf5a87a5` passed push CI `33926105364`, PR CI `33926140785`, Historical Backfill Smoke `33926140869`, Live Recorder Smoke `33926140811`, and Recorder Short Soak `33926140830`; post-merge main CI `33926308381` then passed 956 tests plus rollout syntax, health, and dashboard checks. A follow-up transcript-digest-binding checkpoint on `phase14-storage-preflight-transcript-digest-binding` now hashes the exact raw transcript bytes read by the independent verifier into `transcript_sha256` in verified JSON. The Cloud Shell wrapper independently re-hashes the transcript after verification, requires equality with that embedded digest, removes the verified JSON on mismatch, and prints `TRANSCRIPT_SHA256` only after equality. Clean tests-only RED head `1ca9962d38dc9eaa16db0fce54ce1dea27f44b0d` failed exactly the missing transcript-digest contract while 956 existing tests passed in CI `33926512157`; GREEN head `984a6f41fcfb96809f71c4df03ddb69fe875549d` passed CI `33926698070` with 957 tests plus rollout syntax, health, and dashboard checks. This checkpoint is engineering-only until merged and does not execute the production preflight or authorize migration.
