...
**Complete and integrate the Phase 14 partitioned raw-retention reliability package; keep the production recorder stopped and do not run the production partition migration without a separate explicit authorization.**

Gate A remains production-accepted and the continuous outcome-blind V2 forward-coverage collector was subsequently deployed for research-only collection at production commit `c29fe227f959305f67031e922ca659869a826c4f`. That operational history does not authorize Gate B or any V2 policy/model/calibration/edge selection. The 4 September storage incident interrupted normal recorder operation before those later research stages.

The storage incident established that the Phase 3 monolithic `raw_market_events` table was not physically bounded by row deletion at the observed event volume. The relation reached approximately 157 GB while root free space approached the existing 15 GiB critical reserve, and the maintenance timer had been inactive for several days. The recorder was intentionally stopped fail-closed. A verified 24-hour recovery archive for the 24–48h window was preserved on the dedicated `/mnt/bp-data` disk.

Draft PR #50, `Phase 14 partitioned raw retention reliability`, replaces production physical retirement with hourly PostgreSQL range partitions, preserves global replay dedupe through a 16-way hash-partitioned ledger, retires only verified archived hours by dropping child relations, and adds durable maintenance/retention health. Engineering checkpoint `44ba80fc9e6fdafb6e29c40b0d634d22c86d12e4` passed 903 Python/PostgreSQL tests, Ruff, deployment validation, dashboard tests/typecheck/build, Historical Backfill Smoke, Live Recorder Smoke, and Recorder Short Soak.

The production partition migration has **not** been performed. Its rollout helper is exact-SHA guarded and must fail closed unless the recorder is stopped, research/zero-money gates hold, the protected `/mnt/bp-data` filesystem has sufficient headroom, PostgreSQL and canonical archives are actually on that filesystem, the recovered 24–48h archive evidence verifies, and rollback material can be retained. A successful migration step must still end with `RECORDER_RESTARTED=false`.

The recorder backpressure repair from merged PR #49 is on main at `5da8934bbb0700feb096b2143c53b9ce2d7aa5f9` but was not deployed because the rollout storage precheck correctly failed closed. After a separately authorized partition migration proves exact parity, one archive-to-partition-drop cycle releases physical relation storage, and composite storage health is accepted, the next separate operational step is the already-approved recorder reliability rollout with `RECORDER_WRITER_WORKERS=4` followed by all-four-feed and natural-load no-drop/backpressure acceptance.
