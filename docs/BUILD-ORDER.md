...
**Current status:** Phase 15 has one additional frozen-V3 canary authorized but not submitted. The private Telegram transport is production-staged on both hosts at corrected release head `def15988b87fe90436fa9ee1458e640ba67bb1af` / stage ID `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6`; the read-only stage verifier and Pub/Sub readiness verifier both pass with zero blockers. All transport services remain inactive/disabled, env/key material is absent, dedicated publisher/subscriber identities have no broad project-level Pub/Sub roles, and runtime live trading remains OFF. A prior activation attempt failed closed before completion, submitted no order, and was recovered by rollback/re-stage. Transport activation remains explicitly authorized but not yet activated. The next step is the fail-closed activation helper only; activation must submit no order and must leave the Johannesburg executor safe-idle with the kill switch engaged. After successful activation, exactly one fresh private Telegram APPROVE remains required before the second canary executor action.
...
**Telegram transport stage checkpoint (26 Sep 2026):** corrected restage and independent stage-status checks PASS at `def15988b87fe90436fa9ee1458e640ba67bb1af` / `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6`, and Pub/Sub readiness also PASSes with zero blockers. Evidence is `docs/evidence/phase-15-v3-telegram-transport-restage-readiness-production-20260926.json`. The prior activation attempt failed closed and submitted no order. Transport remains inactive; do not treat staging/readiness as a Telegram approval or order-submission event.
