...
**Treat `docs/evidence/phase-14-v2-freshness-preregistration-20260909.json` as immutable research configuration. The coverage-only input hash is `aab75574aa7faf18e65358353403e5ec1a2b89dd42424eb7b0e3329bf683b099`; `max_last_trade_age_seconds` candidates are frozen as exactly `[1, 2, 5, 10]` plus explicit `no_trade`, with the selected-book freshness ceiling unchanged at 10 seconds. The next build is the separate Gate B V2 research package: construct legitimate chronological V2 partitions and allow timing/model/calibration/edge/min-edge choices only on permitted training/validation context while keeping the final holdout untouched until one evaluation. Do not change the frozen candidate grid after labels/outcomes are joined, and do not treat implementation or validation as Gate B acceptance.**
