...
**Merge and verify the dedicated post-storage recorder restart gate, then run it against the accepted production head `895c6bd2f9409f16bf5d544b26b30e20ecbfe43a` and accepted storage evidence. The gate must select exactly `RECORDER_WRITER_WORKERS=4`, restart only `bp-recorder.service`, prove all four feeds under natural load with no backpressure, preserve partitioned-storage health and research/zero-money safety, and roll back to the stopped recorder state on any failure. The partitioned-storage migration itself is complete and must not be rerun.**
