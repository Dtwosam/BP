...
A follow-up acceptance-evidence durability checkpoint on `phase14-storage-rollout-acceptance-evidence-durability` now flushes the filesystem containing the canonical installed acceptance JSON after generated/installed SHA-256 equality is proven and before temp cleanup or rollback disarm. Clean tests-only RED head `9196c96f626189431f5dde025c68954b4d6f8b89` failed exactly the new durability-ordering contract while 951 existing tests passed in CI `33917015980`; GREEN implementation head `5ac83363476dfe5aff7ab69d66094f5bcb8da299` passed CI `33917183016` with 952 tests plus rollout syntax, health, and dashboard checks. A sync failure fails closed with `rollout_evidence_sync_failed` while rollback remains armed. This checkpoint is engineering-only until merged and does not authorize production execution.
