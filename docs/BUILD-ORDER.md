...
**Do not retry Gate B directly. PR #158 merged the repeatable feature-only readiness checker; sync a clean local checkout to exact current `main` and run `scripts/deploy/phase14_v2_gate_b_readiness_cloudshell.sh` instead. `READY=false` means keep the research-only V2 forward collector running and do not create any Gate B plan/selection/holdout artifact. `READY=true` means the exact frozen planner can currently form the required 8h train / 2h validation / 2h test walk-forward with 2h steps, at least three ordinary folds, and the 2h final holdout under every unchanged market-count minimum. Readiness is repeatable and necessary but is not Gate B authorization; only after `READY=true` may one future exact-main Gate B evidence run be attempted. Never lower minimums, skip folds, alter frozen freshness/edge/cost settings, or use labels/outcomes to decide readiness.**
