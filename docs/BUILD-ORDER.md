...
**Run the merged read-only Phase 14 storage preflight when production access is available; keep the recorder stopped and preserve the separate explicit authorization gate for the actual partition migration.**

PR #50 is merged to `main` at `89b00ba8df4acdfad075cb31df596c86dc148f08`; PR #51, the read-only storage preflight, is merged at `99255f06f329207031071759ce6cb360da518819`. Post-merge CI `33878036846` passed 907 tests plus Ruff, deployment validation, health check, and dashboard tests/typecheck/build.

The read-only helper is `scripts/deploy/phase14_partitioned_storage_preflight_cloudshell.sh`; the runbook is `docs/PHASE-14-STORAGE-RECOVERY.md`. It checks exact deployed/candidate SHAs, research/zero-money safety, stopped-recorder state, `/mnt/bp-data` filesystem identity/headroom, verified 24–48h recovery archive evidence, current PostgreSQL storage shape, and timer states. It does not fetch or check out code, stop/start/enable services, run schema migration or maintenance, write host evidence files, or mutate PostgreSQL.

A preflight PASS is prerequisite evidence only and must end with `MUTATIONS_PERFORMED=false`. The production partition migration remains a **separate explicit production authorization**. When that gate is satisfied, the rollout helper must recheck the safety boundary, retain rollback material, prove exact migration parity, run one verified archive-to-partition-drop cycle, prove physical relation-size release, and end with `RECORDER_RESTARTED=false`.

After storage migration/health acceptance, the next operational step is the already-merged recorder reliability repair with `RECORDER_WRITER_WORKERS=4`, followed by all-four-feed and natural-load no-drop/backpressure acceptance. That recorder restart remains separate from the schema migration.
