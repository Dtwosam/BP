...
**PR #156 merged the Gate B sparse-prefix recovery, but the exact-main production retry still failed safely in feature-only planning: no contiguous epoch yet satisfies the unchanged Phase 8 walk-forward minimums, and all Gate B artifacts remained absent with `HOLDOUT_TOUCHED=false`. Do not retry Gate B yet. Keep the research-only V2 forward collector running and accumulate contiguous immutable `core-v2-last-trade` evidence. The frozen geometry requires at least 18 hours from a viable analysis epoch to fit three ordinary folds plus the final holdout (8h train / 2h validation / 2h test, 2h step, 2h holdout), subject to every unchanged market-count minimum. Resume only when feature-only chronology can satisfy that contract. Do not lower minimums, skip folds, alter frozen freshness/edge/cost search settings, or use labels/outcomes to decide readiness.**
