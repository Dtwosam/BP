...
**Do not retry Gate B or restart the recorder on the currently deployed code. Read-only production diagnostics proved that the 15:00 and 16:00 UTC storage-maintenance cycles deadlocked under recorder load, which made the maintenance heartbeat stale and correctly triggered the 16:05 fail-closed recorder stop. PR #160 fixes the already-partitioned runtime lock inversion by replacing repeated parent/sequence/index DDL with read-only schema validation plus current + two future raw-partition provisioning. Merge only after its exact-head gates pass. Production deployment and recorder recovery remain separate mutations requiring explicit authorization. After an authorized recovery proves the four-worker recorder and storage guards stable again, use only the repeatable exact-main Gate B readiness helper while contiguous V2 evidence accumulates. `READY=false` means keep collecting and do not run Gate B; `READY=true` remains necessary but is not Gate B authorization. Never lower storage thresholds, bypass the recorder storage-health interlock, lower Phase 8 minimums, skip folds, alter frozen research configuration, or touch the final holdout.**
