...
A follow-up verified-preflight single-snapshot checkpoint on `phase14-storage-rollout-preflight-single-snapshot` now reads the verified preflight JSON bytes once, computes its SHA-256 from those bytes, and parses/validates the same snapshot. It removes the former separate `sha256sum` read, closing the local digest/validation TOCTOU gap before gcloud project selection or VM contact. Tests-only RED head `fefa6f0c5d4c5c9d316e2e97cb80eabdb97a1799` failed exactly the new snapshot contract while 943 existing tests passed in CI `33906530270`; GREEN implementation head `b8c669c16ad9029dcb393bf17a6e07ff42e4dc59` passed CI `33906575169` with 944 tests plus rollout syntax, health, and dashboard checks. This checkpoint is engineering-only until merged and does not authorize production execution.
