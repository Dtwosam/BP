...
**Merge the Phase 14 verification-time partition-refresh fix, verify post-merge CI, then freeze `main` and rerun the read-only production storage preflight against that exact new SHA. The `87e08fcf...` rollout mutated storage but rolled back successfully after the current+2 partition window drifted during multi-hour parity verification; its approval/preflight tuple must not be reused. Require a fresh explicit migration approval for the new final SHA/digest tuple.**
