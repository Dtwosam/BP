...
PR #58 adds the missing physical-release acceptance proof. After exact migration parity, the rollout must retire at least one non-empty verified raw partition, prove dedupe cleanup matches the archived row count, and show that total attached raw-partition relation bytes decrease across the maintenance cycle. RED head `a6765fa5774177d5717ea8974e657339681669bc` failed only the new contract while 931 existing tests passed in CI `33896005691`; GREEN implementation head `0305f193e51538d361ab30d18defe1ba9c7c00da` passed CI `33896237045` with 932 tests plus rollout syntax, health, and dashboard checks. This is engineering evidence only; no production migration or approval occurred.
