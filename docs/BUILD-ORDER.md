...
**Merge the maintenance final-partition-window refresh fix, verify post-merge CI, freeze that exact new `main` SHA, rerun the read-only production storage preflight, and require fresh explicit migration approval. The `a7f8418...` attempt proved both exact parity passes, all 23 archive-to-drop retirements, exact 24,482,850-row dedupe cleanup, and the terminal-partial recovery rule in production; it rolled back only because the long maintenance cycle's startup-provisioned current+2 window aged out before final composite health. Do not reuse the prior approval/preflight tuple.**
