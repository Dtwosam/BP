...
**Current status:** Phase 15 has one additional frozen-V3 canary authorized but not submitted. Stage `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6` remains installed but is **not activation-ready** after a second activation attempt failed closed at executor transport service startup. Executor-side fail-closed cleanup is verified: all four Johannesburg transport services are inactive/disabled, its runtime env/key files are absent, the kill switch is engaged, the official account is clean with zero open orders, and no real order was submitted. Recorder-side cleanup remains to be independently revalidated by the guarded rollback preflight. Diagnostics proved two remaining runtime permission defects: the `bp-transport` services could not traverse the extracted release script tree, and the capability-stripped execution-authorization worker could not read the transport-ready queue under the root-only canary parent. The repository recovery patch isolates unprivileged transport state under `/var/lib/bp-telegram-transport`, preserves privileged authorization/handoff state under `/var/lib/bp-canary`, and makes the staged release tree service-readable without widening wallet/canary access. The required next production sequence is rollback of the inactive `c1eb…` stage, corrected restage, read-only stage/Pub/Sub/readiness verification, and only then a fail-closed activation retry. Transport activation remains authorized but activation itself must submit no order. After a successful activation, exactly one fresh private Telegram APPROVE remains required before the second canary executor action.
...
**Telegram transport recovery checkpoint (26 Sep 2026):** the second activation attempt against `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6` failed closed with `executor_transport_service_activation_failed`. Executor-side cleanup and safe-idle are verified and no order was submitted; recorder-side cleanup remains to be revalidated by rollback preflight. The staged package now requires rollback/restage before another activation attempt; do not send Telegram APPROVE from this checkpoint. Evidence: `docs/evidence/phase-15-v3-telegram-transport-second-activation-failure-20260926.json`.
...
PR #114 merged verified-preflight root-filesystem free-space consistency validation to `main` as `ca4d5acaaf0e0bc859b2a9979d41311257ac5445`. Final head `62b5139ba1417cfce2d8917d07b9fe1d9bdd469f` passed push CI `33971541151`, PR CI `33971542837`, Historical Backfill Smoke `33971542839`, Live Recorder Smoke `33971542836`, and Recorder Short Soak `33971542846`; post-merge main CI `33971665075` then passed 976 tests plus rollout/deployment validation, research-mode health, and dashboard checks. The checkpoint is now `MERGED_MAIN_NOT_PRODUCTION_RUN`; no production preflight or migration was executed and no migration authorization was set.

A follow-up verified-preflight archive-window consistency checkpoint on `phase14-storage-rollout-preflight-archive-window-end-consistency` now requires rollout to parse the verified archive `window_end` as ISO-8601 and require timezone-aware semantics before any `gcloud` command, matching the independent verifier's existing output contract. Clean tests-only RED head `08f5132b4e4b9698ae7e890cd238f06f0f544f53` failed exactly this missing contract while 976 existing tests passed in CI `33972005600`; GREEN head `20066b0fb59ed6a3039064132ad6b7be831fef9e` passed CI `33972034131` with 977 tests plus rollout/deployment validation, research-mode health, and dashboard checks. This remains engineering-only until merged; no production preflight or migration was executed and no migration authorization was set.

PR #116 merged verified-preflight archive-window timestamp consistency validation to `main` as `9222f6fd0709d2f0d0c5202191c64bf65670dc7e`. Final head `ea0953b5da96dcc6a5a0fdcbd2bbef0be65702ce` passed push CI `33972159546`, PR CI `33972211650`, Historical Backfill Smoke `33972211651`, Live Recorder Smoke `33972211645`, and Recorder Short Soak `33972211644`; post-merge main CI `33972317705` then passed 977 tests plus rollout/deployment validation, research-mode health, and dashboard checks. The checkpoint is now `MERGED_MAIN_NOT_PRODUCTION_RUN`; no production preflight or migration was executed and no migration authorization was set.

A follow-up verified-preflight zero-money type-consistency checkpoint on `phase14-storage-rollout-preflight-safety-zero-type-consistency` now requires rollout to validate `safety.max_trade_size_usd` and `safety.max_daily_loss_usd` as actual integer zeros before any `gcloud` command, matching the independent verifier's JSON output contract instead of accepting boolean `false` or floating-point `0.0` as zero-equivalent values. Clean tests-only RED head `cfce31a924fab2cc6832bdce174bb0a5cc5a35e7` failed exactly this missing contract while 977 existing tests passed in CI `33972885858`. Initial implementation head `2d1924b166e02886b10de66cbb098f6936085ef6` exposed one stale legacy source-marker assertion in CI `33972909252` (**977 passed, 1 failed**); final repaired head `6afd3257e3a1ea6120bbebff05353166458f74b1` passed CI `33973043120` with 978 tests plus rollout/deployment validation, research-mode health, and dashboard checks. This remains engineering-only until merged; no production preflight or migration was executed and no migration authorization was set.

PR #118 merged verified-preflight integer zero-money safety validation to `main` as `84bf292e9e6cd99fbacf5415098972315c079c63`. Final head `a0332b9fa158b2a1bedf9a4cfa612b48de825480` passed push CI `33973213954`, PR CI `33973192885`, Historical Backfill Smoke `33973192804`, Live Recorder Smoke `33973192826`, and Recorder Short Soak `33973192819`; post-merge main CI `33973295321` then passed 978 tests plus rollout/deployment validation, research-mode health, and dashboard checks. The checkpoint is now `MERGED_MAIN_NOT_PRODUCTION_RUN`; no production preflight or migration was executed and no migration authorization was set.

A follow-up verified-preflight headroom-scalar type-consistency checkpoint on `phase14-storage-rollout-preflight-headroom-scalar-type-consistency` now requires rollout to validate `headroom.minimum_free_gib` and `headroom.critical_reserve_gib` as actual integers before any `gcloud` command, matching the independent verifier's JSON output contract instead of accepting floating-point values that compare numerically equal. Clean tests-only RED head `0567b43434dff6f945282c4f7e6c7b8ff39e50a8` failed exactly this missing contract while 978 existing tests passed in CI `33973216084`. Initial implementation head `5d25e0a0a59007942c1476d5271748e1f49021e0` exposed two stale legacy source-marker assertions in CI `33973224422` (**977 passed, 2 failed**); final repaired head `82b0ea09e45b155c11aa18a761c5ceda472a7783` passed CI `33973275844` with 979 tests plus rollout/deployment validation, research-mode health, and dashboard checks. This remains engineering-only until merged; no production preflight or migration was executed and no migration authorization was set.

PR #120 merged verified-preflight headroom scalar-type validation to `main` as `f7cbec028bec5b615e58941c4ba92ad40db7c7c7`. Final head `2161d055fb907123b7fade4b64ccdf7c194fd059` passed push CI `33973651541`, PR CI `33973665934`, Historical Backfill Smoke `33973665861`, Live Recorder Smoke `33973665867`, and Recorder Short Soak `33973665864`; post-merge main CI `33973798254` then passed 979 tests plus rollout/deployment validation, research-mode health, and dashboard checks. The checkpoint is now `MERGED_MAIN_NOT_PRODUCTION_RUN`; no production preflight or migration was executed and no migration authorization was set.
A follow-up verified-preflight strict-JSON checkpoint on `phase14-storage-rollout-preflight-json-nonfinite-rejection` now rejects `NaN`, `Infinity`, and `-Infinity` through an explicit `json.loads(..., parse_constant=...)` hook before any `gcloud` command. This keeps the locally approved snapshot inside the strict JSON shape emitted by the independent verifier instead of accepting Python-specific non-finite extensions in manually constructed evidence. Clean tests-only RED head `cf18496b798cade7d46da84b95c3fa98b258fa79` failed exactly this missing contract while 979 existing tests passed in CI `33974156401`. Initial implementation head `3f4892d6acabb460dd245db9d58a24f32cb35249` exposed one stale same-snapshot assertion in CI `33974180570` (**979 passed, 1 failed**); final repaired head `e4a65abee20387f9b1a66d4f0980886f6d3c072e` passed CI `33974269257` with 980 tests plus rollout/deployment validation, research-mode health, and dashboard checks. This remains engineering-only until merged; no production preflight or migration was executed and no migration authorization was set.
...
PR #124 merged strict single-snapshot candidate-state binding to `main` as `bfed99b6f8591fcdb7784e5d1399ca2945e8599c`. Final head `d3e8689887927eedbf80cd461158a49063b6bc85` passed push CI `33974867767`, PR CI `33974878464`, Historical Backfill Smoke `33974878459`, Live Recorder Smoke `33974878381`, and Recorder Short Soak `33974878387`; post-merge main CI `33974966377` then passed 981 tests plus rollout/deployment validation, research-mode health, and dashboard checks. The checkpoint is now `MERGED_MAIN_NOT_PRODUCTION_RUN`; no production preflight or migration was executed and no migration authorization was set.
...
PR #134 merged the executable deployed-from mismatch proof to `main` as `ec4a3ffeb1919246cda56e869f3de7cdc0b68df0`. Final head `3b2c163bbdcd8871baf3383f91d86cecb55375ef` passed push CI `33979683145`, PR CI `33979705258`, Historical Backfill Smoke `33979705234`, Live Recorder Smoke `33979705245`, and Recorder Short Soak `33979705267`; post-merge main CI `33979820091` then passed 986 tests plus rollout/deployment validation, research-mode health, and dashboard checks. The checkpoint is now `MERGED_MAIN_NOT_PRODUCTION_RUN`; no production preflight or migration was executed and no migration authorization was set.

PR #136 merged read-only preflight observability and worker-stdin hardening to `main` as `c9d94041993b59e41642faff684dd031cbe71012`. A production read-only attempt against candidate `fb5def00896777c505377615e389f64088b127d1` reached the required research/zero-money checks but then exited 0 without the mandatory PASS marker, so it is explicitly unaccepted and must be rerun. The production origin is HTTPS (`https://github.com/Dtwosam/BP.git`). The hardening captures host stderr in the evidence transcript and removes the `bash -s` worker-source stdin stream entirely: worker code is passed as command data and executed with remote stdin at `/dev/null`, preventing child processes from consuming the remainder of the worker program. Final head `ab7713b594703f6672eb108b138a7f78143bc6ff` passed push CI `33983797325`, PR CI `33983799393`, Historical Backfill Smoke `33983799495`, Live Recorder Smoke `33983799373`, and Recorder Short Soak `33983799420`; post-merge main CI `33983907097` passed 989 tests plus rollout/deployment validation, research-mode health, and dashboard checks. No production mutation or migration authorization occurred.
...
The authorized Gate B attempt from `0941684676ea35949ac938a3bca33012b6ea0e19` wrote `plan.json` and then failed in `prepare` on a missing canonical non-holdout label. That plan was frozen for the narrowly authorized recovery/resume sequence. The sequence is now complete: the later resume durably wrote `holdout-attempt.json`, touched the final holdout, and failed closed on missing canonical supervised input. The frozen plan and its final holdout are consumed and must not be reused.

Superseded order: do not rerun audit/recover/resume for frozen plan SHA-256 `8f2a756161bb0d85e6020d6ff0d6f4f3540eb28caf133870a4926f65ac7d2fea`; preserve that evidence and its `holdout-attempt.json` marker as immutable failure evidence. Next order: (1) treat PR #179 production rollout as a separate production-mutation boundary; (2) after any accepted rollout, build a fresh statistically clean V2 Gate B plan from feature-only readiness with a new final holdout; (3) require a new one-shot explicit authorization before any final-holdout access. No holdout cherry-pick, automatic promotion, Phase 15, live trading, or money change is permitted.

PR #172 merged the hardened recovery engineering to `main` as `3e00939f7f70453f9aa054e46c31c72600cdb83d` from final head `1f6709632b3a5546bdd158552d8670ca7603bd1b`. All four exact-head gates passed (CI `34479665673`, Historical Backfill Smoke `34479665575`, Live Recorder Smoke `34479665644`, Recorder Short Soak `34479665747`), and post-merge CI `34480403106` passed 1,059 tests. The final helper also persists a durable `holdout-attempt.json` marker before any final-holdout evaluation and records Gamma provenance at response receipt time. The later production label recovery and same-plan resume were separately authorized and performed; the resume consumed the final holdout, so the frozen plan is no longer reusable.

## Phase 14 partition-local dedupe cleanup engineering — 10 September 2026

A read-only frozen-plan Gate B label-recovery audit reached production using helper/main head `4c3c8be828a0a5cfdab04abad36bca32bb2df7db` and exact frozen plan SHA-256 `8f2a756161bb0d85e6020d6ff0d6f4f3540eb28caf133870a4926f65ac7d2fea`, but failed closed before any label inspection because composite partitioned-storage health was not `ok`. No Gate B labels or final holdout were read, no Gate B artifact was created, and the audit performed no production database mutation.

Read-only diagnostics found the hourly maintenance path spending its bounded 50,000-row dedupe cleanup batches on a global parent-table `DELETE` whose PostgreSQL plan hash-joined/scanned approximately 24.4 million `raw_event_dedupe` rows across all 16 hash partitions per batch. The previous 13:00 UTC cycle exceeded the 55-minute service timeout and the 14:00 cycle again ran long enough for the two-hour maintenance-freshness guard to become false. Disk free space remained above the configured 15 GiB critical reserve; composite health became critical because maintenance freshness expired, not because the filesystem crossed the critical reserve.

PR #174 replaced that global cleanup with partition-local indexed `ctid` batches while preserving archive verification, raw-partition-drop-before-dedupe ordering, exact deletion accounting, retention windows, and all research/trading safeguards. Final head `1e650786b3f8320d8406a84fb8e232bd7548a2ba` passed CI `34499860031`, Historical Backfill Smoke `34499860145`, Live Recorder Smoke `34499860123`, and Recorder Short Soak `34499860179`; it merged to `main` as `ad71eb4d72948a023e57eca51c99ebce700df0a6`, and post-merge CI `34500104025` passed **1,060 tests** plus Ruff, deployment validation, research-mode health, and dashboard checks.

The fix is merged engineering only and is not yet production-deployed. Before another Gate B label audit, production storage maintenance must be deliberately updated/recovered through a separate exact-SHA production authorization and must return fresh composite storage health `ok`. Do not lower storage thresholds or bypass maintenance freshness. Only then rerun the same holdout-blind audit against the existing frozen plan; label recovery and same-plan holdout resume remain separate fresh authorization boundaries.


## Phase 14 dedupe cleanup production-recovery gate — 10 September 2026

The production-shaped recovery candidate is frozen at `71b33d3beaba4a11ef93e7c5bde1c517323f3440`, based directly on currently deployed `e9c7afc1536880e4612cb6e3d1a7282fa37c69f5` and changing only `src/bp_engine/storage/maintenance.py` plus `tests/storage/test_partitioned_raw_postgres.py`. Verification-only PR #176 passed CI `34504013943` and was closed without merge.

PR #177 added `scripts/deploy/phase14_dedupe_cleanup_recovery_gate_cloudshell.sh`, binding production recovery to the exact deployed/candidate heads and accepted storage-evidence digest. Final head `75584201921a7b0b72a287ded310f4f3959d6647` passed CI `34504578757`, Historical Backfill Smoke `34504578825`, Live Recorder Smoke `34504578698`, and Recorder Short Soak `34504578864`; it merged as `a09837c0304325d783a7cc94f552b35665435640`. Post-merge CI `34504827764` passed **1,065 tests** plus Ruff, deployment validation, research-mode health, and dashboard checks.

Production is unchanged. The recovery helper requires a fresh explicit approval string binding the final helper/main SHA, deployed SHA `e9c7afc1536880e4612cb6e3d1a7282fa37c69f5`, candidate SHA `71b33d3beaba4a11ef93e7c5bde1c517323f3440`, and storage evidence SHA-256 `f33a28f5306e46c509b0000a176d226c079aa2d160d595095ca228118542ce19` before any cloud contact. After accepted recovery, rerun the existing frozen-plan Gate B label audit read-only. Label repair and same-plan holdout resume remain separate authorization boundaries.

## Phase 14 V2 outcome-label coverage rollout PASS — current order
