...
A follow-up acceptance-evidence digest checkpoint on `phase14-storage-rollout-acceptance-evidence-digest-v2` now computes SHA-256 from the installed final rollout evidence, validates the digest shape, and prints `EVIDENCE_SHA256` before rollback is disarmed. Clean RED head `ac0b2eb17e1c5d689abfbcf00af83fa2d6e4c548` failed exactly the new digest-ordering contract while 949 existing tests passed in CI `33913585370`; GREEN implementation head `b923649810de950a29843d0436a006f54cd4c62d` passed CI `33913739000` with 950 tests plus rollout syntax, health, and dashboard checks. This checkpoint is engineering-only until merged and does not authorize production execution.
