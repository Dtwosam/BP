...
**Production recovery is now explicitly authorized for the narrow Phase 14 recorder deadlock scope. Execute only `scripts/deploy/phase14_recorder_deadlock_recovery_gate_cloudshell.sh` from exact **current** `origin/main`; the helper must reject if remote main changes and must prove the pinned candidate runtime/test blobs remain byte-identical to that execution head. The helper is bound to accepted production head `895c6bd2f9409f16bf5d544b26b30e20ecbfe43a`, exact minimal candidate `e9c7afc1536880e4612cb6e3d1a7282fa37c69f5`, the accepted partitioned-storage evidence digest, existing `RECORDER_WRITER_WORKERS=4`, RESEARCH/live-disabled/zero-money safety, and the unread final-holdout boundary. Accept only `PHASE14_RECORDER_DEADLOCK_RECOVERY_GATE=PASS` with its host evidence artifact. On any failure, preserve the helper-defined rollback state with the recorder stopped. This authorization does **not** authorize Gate B, final-holdout access, policy/model/calibration/edge selection, automatic promotion, Phase 15, live trading, or nonzero money limits.**
