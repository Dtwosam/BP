...
**The Gate B package is merged, but the first production evidence attempt failed safely in feature-only planning before labels or final holdout because the earliest two-hour test window had 4 markets versus the unchanged Phase 8 minimum of 6. Review and merge `phase14-v2-gate-b-sparse-prefix-recovery`, then retry from exact `main`. The recovery may move only the feature-only analysis epoch start in the existing two-hour cadence; it must choose the earliest start whose entire remaining contiguous walk-forward schedule satisfies every unchanged Phase 8 minimum. It must not skip individual folds, lower minimums, inspect labels for epoch selection, change the frozen freshness/edge/cost search configuration, or touch the final holdout before `prepare` freezes the policy. The prior attempt created only the partial run directory; `plan.json`, `selection.json`, and `holdout.json` were not written, so the one-shot holdout remains unread and a post-fix retry is permitted.**
