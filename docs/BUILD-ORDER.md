...
On 23 September 2026 the user explicitly authorized pursuing a controlled live transition for the exact frozen V3 while V4 research continues. That changes only the human-authorization row. The V3-specific read-only analyzer is implemented, and the production-safe bridge is `bash scripts/deploy/phase14_v3_live_gate_reassessment_cloudshell.sh`. After the bridge merges green, run it once from exact clean current `main`; it must isolate `v3-frozen-paper-v1` / `paper-execution-v3-frozen-v1`, report uncertainty/drawdown/outlier sensitivity/calibration/reconciliation, call exactly the official direct Polymarket geoblock endpoint from the production VM, and leave live trading disabled, Phase 15 blocked, and real-money limits at zero. A bridge PASS is evidence collection only. A separate complete Master live-gate decision is mandatory and geographic restrictions may not be bypassed.
