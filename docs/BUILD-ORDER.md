...
**Review and merge the engineering-ready V2 Gate B research package from `phase14-v2-gate-b-research`, then run the exact merged `main` package read-only against production evidence. The run must preserve the three-stage boundary: (1) feature-only `plan` freezes chronological whole-market partitions, final holdout, embargoes, fee/slippage assumptions, V2 edge grid, and minimum eligibility/trade counts before any label query; (2) `prepare` may read only non-holdout labels and must freeze timing/freshness/calibration/edge choices from validation before ordinary-test evaluation; (3) `evaluate-holdout` may read final-holdout labels once and cannot rewrite the frozen selection. All artifacts are hash-bound and no-clobber. The package is read-only, does not deploy to `/opt/bp`, does not mutate production registries, and must keep `gate_b_authorized=false` / `automatic_promotion=false` until the resulting evidence is separately reviewed and accepted.**
