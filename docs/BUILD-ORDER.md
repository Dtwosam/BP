...
**Keep `bp-recorder.service` stopped and do not run Gate B. PR #160 is merged to main as `a903cf4bb1cb2e2a24e80a8b00f0eecb68b3e15a`, but production must not jump from accepted head `895c6bd2f9409f16bf5d544b26b30e20ecbfe43a` across the 153 later commits merely to obtain the storage fix. The exact minimal production candidate is `e9c7afc1536880e4612cb6e3d1a7282fa37c69f5` on `ops/phase14-storage-deadlock-recovery-candidate`; its diff is only the exact-main `partitioned_raw.py` fix plus its PostgreSQL regression test, and candidate CI is green. Review and merge `scripts/deploy/phase14_recorder_deadlock_recovery_gate_cloudshell.sh` and its runbook/tests first. Even after that engineering merge, production checkout mutation and recorder start remain separately unauthorized until explicit approval. A future authorized PASS must prove an actual maintenance cycle succeeds under active four-worker recorder load, all four feeds remain healthy with zero backpressure, storage remains `ok`, timers are restored, and Gate B artifacts remain unchanged. Only after that recovery may the repeatable feature-only Gate B readiness helper resume; `READY=true` is still not Gate B authorization and the final holdout must remain untouched.**
