...
**Keep the recovered four-worker recorder and the accepted storage/V2 timers running under normal RESEARCH/live-disabled/zero-money safeguards. The latest accepted feature-only Gate B readiness result remains `READY=false` from `2026-09-09T20:13:53Z`; Gate B and the final holdout remain blocked. PR #167 merged a hardened two-hour readiness watcher to `main` as `80bf77da54fd1a2c89b4d43910457184bd06fead`: final head `0513770c824baf91f82de74e2af6b4789ccaa7d9` passed CI `34410923330` with 1,039 tests, Historical Backfill Smoke `34410923248`, Live Recorder Smoke `34410923329`, and Recorder Short Soak `34410923305`; post-merge CI `34411184671` also passed 1,039 tests. The watcher is engineering-ready but **not production-installed and not authorized for installation**. Until a separate production-install authorization exists, continue using only the exact-main manual feature-only readiness helper at the useful two-hour cadence. The merged watcher does not change `/opt/bp`, does not restart the recorder/core services, uses PostgreSQL `SET TRANSACTION READ ONLY`, refuses to run if any Gate B artifact exists by pathname, and cannot turn `READY=true` into Gate B authorization. Full contract: `docs/PHASE-14-V2-GATE-B-READINESS-WATCH.md`.**
