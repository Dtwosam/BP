...
**Current status:** Phase 15 has one additional frozen-V3 canary authorized but not submitted. Corrected Telegram transport stage `phase15-telegram-stage-05b83214b159772872bc7347` remains staged, and read-only stage/listener/PubSub/activation-readiness checks pass with Johannesburg safe-idle preserved. The third transport activation attempt failed closed after mutation began because the activation helper wrote raw binary transport/origin keys while both runtime loaders require one-line UTF-8 base64url keys. Post-failure diagnostics verified complete executor and recorder cleanup, kill-switch engagement, zero open orders, no live order, healthy listener state, and no need to restage. The candidate helper now writes/validates base64url key text and fixes the fail-closed kill-marker newline; its exact Git blob is `83157c6c04b9a996bab51012b4bda4dd31062320`. Because the previous activation authorization was bound to helper blob `d2560354d10c9964ba0a5a0161f1be9ffb2dfd86`, transport activation is now `REAUTHORIZATION_REQUIRED`. No activation retry is permitted until fresh explicit authorization is recorded for the exact new helper artifact. Telegram APPROVE, executor arming, and order submission remain later separate boundaries.
