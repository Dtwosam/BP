...
**PR #156 merged the Gate B sparse-prefix recovery and post-merge CI passed. Sync a clean local checkout to the exact current `main` head and retry the read-only Gate B production-evidence helper. Keep every Phase 8 duration/count minimum and the frozen V2 freshness/edge/cost search configuration unchanged. The planner may exclude only a feature-only sparse prefix by choosing the earliest two-hour-step analysis start whose entire remaining contiguous schedule is eligible; it may not skip individual folds or use labels to choose the epoch. The prior attempt failed before `plan.json`, `selection.json`, or `holdout.json` existed, so the final holdout remains unread and one retry is safe. On any new failure, inspect the helper's explicit `PLAN_PRESENT`, `SELECTION_PRESENT`, `HOLDOUT_PRESENT`, `SUMMARY_PRESENT`, and `HOLDOUT_TOUCHED` fields; if `HOLDOUT_TOUCHED=true`, do not rerun.**
