...
A follow-up final storage-health recheck on `phase14-storage-rollout-final-health-recheck` now factors the composite partitioned-storage health validation into a reusable helper, runs it after the maintenance/physical-release cycle, and reruns it after managed-unit restoration plus final rollback-material verification immediately before PASS evidence. Acceptance evidence therefore embeds the post-restoration health snapshot rather than a stale pre-restoration result. Clean tests-only RED head `69bf5c43cdbce9119b0b7656c8547dd4659207c2` failed exactly the new ordering contract while 948 existing tests passed in CI `33913074098`; GREEN implementation head `b20967d0c7ee7bd429aec31c1a522b725320a98a` passed CI `33913033605` with 949 tests plus rollout syntax, health, and dashboard checks. This checkpoint is engineering-only until merged and does not authorize production execution.
