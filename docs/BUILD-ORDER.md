...
**Partitioned-storage migration and storage health are now accepted in production. The next operational gate is the separate controlled recorder restart using the already-merged `RECORDER_WRITER_WORKERS=4` reliability repair. Before treating the recorder as recovered, verify all four required feeds under natural load and prove no dropped writes/backpressure regression. Keep Gate B unauthorized, selected-book freshness at exactly 10 seconds, `automatic_promotion=false`, Phase 15 blocked, and live trading disabled.**
