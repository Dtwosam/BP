...
**Current status:** Phase 15 has one additional frozen-V3 canary authorized but not submitted. Corrected Telegram transport stage `phase15-telegram-stage-05b83214b159772872bc7347` is staged and its transport activation is explicitly authorized but not activated. The latest activation attempt stopped before any mutation because the prerequisite Telegram approval listener failed its strict status gate. Production diagnostics show the enabled listener in a restart loop with `ModuleNotFoundError: No module named 'bp_engine.execution.telegram_approval'` and restart counter 1749; static Telegram env/unit/state metadata remain correct and handoff is absent. Repository remediation is prepared to make the minimal listener release hermetic/service-readable and to add a dedicated listener-only runtime repair helper. Source truth marks listener repair required and not yet authorized. The listener runtime repair is now explicitly authorized for exact reviewed main `a60ff465ab307a360fb20fcefa5158f8258e46a8` and may be executed with the reviewed listener-only repair helper. Transport activation must not be retried until listener repair and read-only status pass. Telegram APPROVE, executor arming, and order submission remain separate later boundaries.
