...
**Keep `bp-recorder.service` stopped and do not run Gate B. PR #162 merged the purpose-built post-acceptance recorder deadlock recovery gate to `main` as `63a0eb8b52f736db4b3e329adcdec06ae2ae02f9` after exact-head CI `34388036133`, Historical Backfill Smoke `34388035993`, Live Recorder Smoke `34388035795`, and Recorder Short Soak `34388035889` all passed; post-merge CI `34388363196` then passed 1,028 tests plus deployment validation, research-mode health, and dashboard checks. The exact minimal production candidate remains `e9c7afc1536880e4612cb6e3d1a7282fa37c69f5`, changing only the exact-main partitioned-storage runtime fix and its PostgreSQL regression test relative to accepted production head `895c6bd2f9409f16bf5d544b26b30e20ecbfe43a`. This engineering merge is **not** production authorization: checkout mutation and `bp-recorder.service` start require a separate explicit approval. Until then preserve `RECORDER_WRITER_WORKERS=4`, all storage thresholds and fail-closed interlocks, RESEARCH/live-disabled/zero-money safety, `automatic_promotion=false`, and the unread final-holdout boundary. After a future authorized recovery PASS, resume only the feature-only Gate B readiness helper; `READY=true` still is not Gate B authorization.**
