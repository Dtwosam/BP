...
A follow-up approval/preflight binding checkpoint on `phase14-storage-rollout-approval-preflight-digest` now requires explicit production-migration approval to include `PHASE14_PARTITIONED_STORAGE_APPROVED_PREFLIGHT_SHA256`, an exact 64-character digest matching the single-snapshot verified-preflight SHA-256. This prevents an approval for the same Git transition from being silently reused with different preflight evidence or a different verified target/archive snapshot. Tests-only RED head `e72aa9214553d1fd8d01abbdafcdcc0bb9b11a19` failed exactly the new approval-digest contract while 944 existing tests passed in CI `33907153503`; GREEN implementation head `e33e9c6a80339a56e11db2546f524b7a8fae9157` passed CI `33907187924` with 945 tests plus rollout syntax, health, and dashboard checks. This checkpoint is engineering-only until merged and does not authorize production execution.
