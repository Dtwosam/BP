...
**Merge the validated recovery-only terminal-partial partition retirement fix, verify post-merge CI, freeze that exact new `main` SHA, rerun the read-only production storage preflight, and require fresh explicit migration approval. The `f5609891...` rollout proved both full 24,482,850-row parity passes, then rolled back safely because the final partially populated raw hour could not satisfy the steady-state full-hour compact watermark while the recorder was intentionally stopped. Do not reuse the prior approval/preflight tuple.**
