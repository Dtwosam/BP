...
A further rollout chain-of-custody checkpoint on `phase14-storage-rollout-archive-evidence-binding` binds the host recovery archive to the exact evidence referenced by the verified preflight. The launcher extracts the preflight archive evidence name and `window_end`, passes both into the detached worker, and the worker requires that exact canonical evidence file and matching window instead of selecting the newest recovery JSON with `ls -1t`. RED head `aa2ad57931b9d6fc9913e6f41465b0ec6b3e6207` failed only the new contract with 933 existing tests passing; GREEN head `8738102bb78c5b036c9e0fc9e41e8f5360a5bcd7` passed CI `33898435814` with 934 tests plus rollout syntax, health, and dashboard checks. This is engineering-only until merged and does not authorize production execution.
