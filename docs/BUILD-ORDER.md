...
**Current status:** Phase 15 has one additional frozen-V3 canary authorized but not submitted. Corrected Telegram transport stage `phase15-telegram-stage-05b83214b159772872bc7347` remains staged, read-only healthy, and does not require restage. The third activation failure root cause was the old helper writing raw binary transport/origin keys where the runtime requires one-line UTF-8 base64url values; fail-closed cleanup was verified on both hosts with safe-idle, zero open orders, and no live order. PR #310 merged the repaired activation helper to current main `23372ef1b9038a756a8187209873de363498aae0`; exact helper blob is `83157c6c04b9a996bab51012b4bda4dd31062320`. The user has now explicitly reauthorized transport activation for exactly this stage/main/helper binding, so source truth is `AUTHORIZED_NOT_ACTIVATED`. The next production step is to rerun the read-only current-main/listener/stage/PubSub/activation-readiness gates and, only if they pass, run the fail-closed transport activation. Telegram APPROVE, executor arming, and order submission remain later separate boundaries.
