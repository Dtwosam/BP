...
**After this validated rollout-scope fix lands on `main`, verify post-merge CI, freeze that exact `main` SHA, and rerun the read-only production storage preflight. The `cc61d290...` approval/preflight tuple failed safely before mutation and must not be reused; require a fresh explicit migration approval for the new final SHA/digest tuple.**
