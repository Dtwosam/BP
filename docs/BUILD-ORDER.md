...
A follow-up read-only preflight binding checkpoint on `phase14-storage-preflight-state-archive-binding` now derives the canonical recovery evidence path from the clean candidate `PROJECT_STATE.json`, passes that exact path to the remote preflight, and removes the previous newest-by-mtime archive selection. RED head `ba0a4462d5ac22f60ab5a99a683d423604adf350` failed exactly two new binding contracts while 939 existing tests passed in CI `33904374260`; GREEN implementation head `05833429f1ed7445a7b663a40978d48f1a8555a3` passed CI `33904435994` with 941 tests plus both preflight shell syntax checks, health, and dashboard checks. This checkpoint is engineering-only until merged and remains read-only; it does not authorize production execution.
