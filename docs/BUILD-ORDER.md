...
**Do not retry Gate B directly. Review and merge the feature-only Gate B readiness checker, then use its exact-main Cloud Shell helper repeatedly while the research-only V2 forward collector accumulates contiguous immutable evidence. `READY=false` means continue collecting and do not create any Gate B plan/selection/holdout artifact. `READY=true` means the exact frozen planner can currently form the required 8h train / 2h validation / 2h test walk-forward with 2h steps, at least three ordinary folds, and the 2h final holdout under every unchanged market-count minimum. Readiness is necessary but not Gate B authorization; only after `READY=true` may one future exact-main Gate B evidence run be attempted. Never lower minimums, skip folds, alter frozen freshness/edge/cost settings, or use labels/outcomes to decide readiness.**
