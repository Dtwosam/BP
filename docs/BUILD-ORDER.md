...
**After this validated recovery-only terminal-partial retirement fix lands on `main`, verify post-merge CI, freeze that exact new `main` SHA, rerun the read-only production storage preflight, and require fresh explicit migration approval. The `f5609891...` rollout proved both full 24,482,850-row parity passes, then rolled back safely at the stopped-recorder terminal compact boundary; its approval/preflight tuple must not be reused.**
