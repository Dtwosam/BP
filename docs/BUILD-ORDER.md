...
**Current status:** Phase 15 single-order canary engineering. The complete Master live gate is `pass`, including the later direct geography checks from the user's ordinary network (`NG/LA`) and the dedicated Johannesburg execution host (`ZA/GP`). Historical US-host geoblock failures remain immutable evidence; the US host must never submit authenticated Polymarket orders.

The only authorized live scope is `phase15-v3-single-order-canary-v1`. It permits exactly one external order-submission attempt using the unchanged frozen V3 signal and the existing $5 target notional. Risk is capped at $5 max trade size, $5 total exposure, $5 daily loss, and one consecutive loss. Both hosts use kill switches, and the Johannesburg executor atomically consumes the one-attempt marker before the official SDK submission call.

Deployment remains a separate explicit production mutation. It must require `PHASE15_ACCEPT_REAL_MONEY_CANARY=yes` and `PHASE15_CANARY_MAX_LOSS_USD=5`, keep the global US production environment research/zero-money, store the trading secret only on Johannesburg, validate the exact-SHA short-lived activation manifest and fresh direct geoblock, and unlock the two kill switches only as the final step.

After the first external attempt, stop. Reconcile the external order/position before any later canary or broader live rollout. Do not automatically retry, increase stake, tune V3, or change V4.
...
1. Validate and merge the exact frozen-V3 single-order canary package. Do not deploy from an unmerged branch.
2. Deploy the canary only with the explicit $5 real-money acknowledgement. The existing US host remains research/zero-money globally; authenticated Polymarket signing/submission occurs only on the unblocked Johannesburg execution host.
3. Allow at most one external submission attempt. After that attempt, preserve both kill switches and manually reconcile the external order/position before any future live authorization.
4. Continue frozen V3 paper observation and the existing **V4 regime-aware feature collector** unchanged. Do **not** run V4 readiness/planning before `2026-09-30T00:00:00Z`, and do not tune V3 from the live canary.
