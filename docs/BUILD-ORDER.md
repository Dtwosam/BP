...
A verified preflight PASS is prerequisite evidence only. The production partition migration remains a **separate explicit production authorization**. The rollout-hardening follow-up on `phase14-storage-rollout-preflight-parity` now requires the verified preflight JSON itself, binds it to the exact deployed/candidate SHAs, records its SHA-256 in rollout evidence, and independently recomputes the dynamic migration-headroom rule before mutation and again immediately before migration apply. Engineering head `c6bc79ab6a832d26d421bd37797156c0b9219b8c` passed CI `33887804985` with 929 tests plus Ruff, deployment validation, health, and dashboard checks.
