...
A follow-up archive-byte binding checkpoint on `phase14-storage-rollout-archive-digest-binding` now captures the canonical recovery evidence SHA-256 during the read-only preflight, requires the verifier to preserve that exact digest, and makes the rollout worker re-hash the preflight-bound host file before mutation. A same-named file with changed bytes fails closed with `archive_evidence_digest_mismatch`. Clean tests-only RED head `497e001d57a5dcc65237fec11dc67b7089c5995a` failed exactly four new digest-contract tests while 933 existing tests passed in CI `33901674705`; GREEN implementation head `b874b5215d171d181cad2b1626a60b36e91a5e78` passed CI `33901670503` with 937 tests plus preflight/rollout syntax, verifier compilation, health, and dashboard checks. This checkpoint is engineering-only until merged and does not authorize production execution.
