...
A further rollout-launcher hardening checkpoint on `phase14-storage-rollout-local-candidate-binding` binds the Cloud Shell helper itself to the exact candidate checkout before any gcloud interaction. The helper resolves the local repository root, requires local `HEAD == PHASE14_PARTITIONED_STORAGE_HEAD`, and rejects any tracked or untracked working-tree change. This prevents an older or locally modified rollout launcher from targeting a newer candidate whose safety gates differ. RED head `558475a6d29421cf15477cc012c10b086897b020` failed only the new contract with 932 existing tests passing; GREEN head `fff7adf984259b28a03b20ceff6d06cd906f5153` passed CI `33897354206` with 933 tests plus rollout syntax, health, and dashboard checks. This is engineering-only until merged and does not authorize production execution.
