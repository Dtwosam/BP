...
A follow-up final rollback-material proof on `phase14-storage-rollout-final-rollback-proof` now captures the original `raw_market_events` PostgreSQL relation OID during the existing pre-migration headroom checks. After the partition retirement/physical-release cycle and service restoration, the worker requires `raw_market_events_legacy` to retain that exact OID and nonzero relation bytes before acceptance evidence can be written; the evidence records original/retained OIDs and retained bytes. This avoids a full-table scan while proving the original rollback relation object survived through final acceptance. RED head `47ab3d8fe9fb588eb4a263bc8be46b5b9c238afa` failed exactly the new rollback-proof contract while 947 existing tests passed in CI `33910240165`; GREEN implementation head `40799f0201f0fcd60659c1a21b7ea61e30101989` passed CI `33910354157` with 948 tests plus rollout syntax, health, and dashboard checks. This checkpoint is engineering-only until merged and does not authorize production execution.
