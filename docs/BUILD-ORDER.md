...
**After this validated verification-time partition-refresh fix lands on `main`, verify post-merge CI, freeze that exact `main` SHA, and rerun the read-only production storage preflight. The `87e08fcf...` rollout mutated storage but rolled back successfully after the current+2 partition window drifted during multi-hour parity verification; its approval/preflight tuple must not be reused. Require a fresh explicit migration approval for the new final SHA/digest tuple.**
