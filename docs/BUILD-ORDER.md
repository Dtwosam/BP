...
**Current status:** Phase 15 has one additional frozen-V3 canary authorized but not submitted. Telegram transport stage `phase15-telegram-stage-05b83214b159772872bc7347` is now production-active after the repaired fail-closed activation helper passed from exact main `469049a9f6361f9c656e3d176029b10db7359689` with helper blob `83157c6c04b9a996bab51012b4bda4dd31062320`. Listener, stage, Pub/Sub, and activation-readiness gates all passed first with zero blockers. Activation reused the existing topic/subscription, established new transport/origin key IDs, started+enabled the publisher and all four Johannesburg transport services, kept global and Phase-15 live trading disabled, left the executor safe-idle, and submitted no real order. The transport-activation authorization is consumed and any retry requires fresh explicit authorization. The current boundary is `WAITING_FOR_FRESH_TELEGRAM_APPROVAL`, but there is not yet a pending intent. The already-authorized persistent prepare-only watcher starter has been aligned to this Telegram second-canary state (candidate blob `7becabcb3768d30988c082fe77b51e163ffddd6f`); it remains research/zero-money and cannot arm or submit. The next production step is to start that watcher and wait for exactly one fresh NEW frozen-V3 paper-derived $5 intent, then review it before any Telegram APPROVE. Telegram APPROVE, executor invocation, and order submission remain unperformed, and official reconciliation remains mandatory after any future one-shot execution.
...
**Telegram transport restage checkpoint (26 Sep 2026):** inactive failed stage `phase15-telegram-stage-c1eb8ce238d6cd96d2edaec6` was rolled back PASS on both hosts and corrected stage `phase15-telegram-stage-05b83214b159772872bc7347` was installed from exact main `ba0bc324cab50bf0809c8eed4e019c13bc4c6653`. Read-only stage status and Pub/Sub readiness both PASS with zero blockers; services remain off and no order was submitted. Transport activation is not authorized for this new stage and requires a fresh explicit authorization. Evidence: `docs/evidence/phase-15-v3-telegram-transport-restage-05b832-readiness-production-20260926.json`.
