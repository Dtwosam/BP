...
**Current status:** Phase 15 has one additional frozen-V3 canary authorized but not submitted. The private Telegram transport is now production-staged on both hosts at release head `facf1dbea1e745419f9e615e0692c9acef952b11` / stage ID `phase15-telegram-stage-3fe604820deabe0c8971f23c`, with the read-only stage verifier passing zero blockers. All transport services remain inactive/disabled, env/key material is absent, IAM/Pub/Sub are unchanged, and runtime live trading remains OFF. Transport activation was explicitly authorized on 25 Sep 2026 but has not yet completed. Before the first transport-activation mutation, install the currently absent Telegram approval listener on `bp-recorder` with the explicitly authorized interactive installer and require read-only listener-status PASS with handoff still disabled. Then remediate only the reviewed dedicated VM service-account/`cloud-platform` scope prerequisites with `scripts/deploy/phase15_v3_telegram_transport_identity_remediation_cloudshell.sh`; this controlled step restarts each VM while preserving transport inactivity and executor safe-idle. Rerun read-only Pub/Sub readiness and, if only activation-owned resource blockers remain, run the fail-closed activation helper. Activation must submit no order and must leave the executor safe-idle with the kill switch engaged.
...
**Telegram transport stage checkpoint (25 Sep 2026):** the stage-only install and read-only stage status both passed on production. Evidence is `docs/evidence/phase-15-v3-telegram-transport-stage-production-20260925.json`. Do not activate from this checkpoint. A separate explicit activation authorization is required before VM identity/scope remediation, Pub/Sub/IAM/key provisioning, or any transport service start/enable.
