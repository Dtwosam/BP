...
Run the rollout helper only from a clean local BP checkout whose `HEAD` exactly equals `PHASE14_PARTITIONED_STORAGE_HEAD`. The helper fails before any gcloud interaction on a local candidate-head mismatch or any tracked/untracked working-tree change, so stale or locally modified launcher code cannot be used for a newer candidate. It now also requires the verified-preflight file to be locally readable, parses and validates the complete single-snapshot preflight binding with duplicate JSON object keys rejected recursively and non-standard `NaN`/`Infinity` constants rejected, and matches the explicit approved-preflight SHA-256 before invoking even the `gcloud auth list` probe. Invalid, ambiguous, stale, or differently approved local evidence therefore fails entirely locally before cloud tooling is invoked.
...
When migration is separately and explicitly approved, the rollout helper now requires `PHASE14_PARTITIONED_STORAGE_PREFLIGHT_VERIFIED` to point to the readable absolute path of the verified JSON produced by the preflight operator. Before it contacts the production VM, it fails closed unless that JSON reports `PASS`, matches the exact deployed-from and candidate/remote SHAs, carries `remote_branch` equal to the rollout's `PHASE14_PARTITIONED_STORAGE_BRANCH`, carries `env_file` exactly equal to the rollout's `PHASE14_PARTITIONED_STORAGE_ENV_FILE`, preserves the verified research/zero-money `safety` block (`mode=research`, `live_trading_enabled=false`, integer-zero trade/loss limits, and `automatic_promotion=false`), reports `mutations_performed=false`, confirms `RECORDER_STATE=stopped`, identifies `storage_shape=legacy_unmigrated`, and requires the verifier's underlying `raw` block to agree with that shape (`partitioned=false`, `legacy_table_present=false`, `dedupe_table_present=false`, and non-negative integer `estimated_rows`), carries `PROJECT` / `ZONE` / `VM` target identity that exactly matches the rollout target, carries integer `headroom.minimum_free_gib` exactly equal to the rollout's `PHASE14_PARTITIONED_STORAGE_MIN_FREE_GIB`, and carries the unchanged integer `headroom.critical_reserve_gib=15` used by both the official preflight and rollout live headroom rechecks. The same local validation now also requires positive integer `headroom.free_bytes`, `headroom.root_free_bytes`, `headroom.raw_total_bytes`, and `headroom.required_free_bytes`, recomputes `max(configured minimum, raw total bytes + 15 GiB)`, requires the verified required bytes to equal that result, and requires verified dedicated-data free bytes to meet it. A PASS captured from a different project, zone, VM, environment-file path, safety boundary, configured migration floor, reviewed critical reserve, contradictory raw-storage shape evidence, or internally inconsistent migration-headroom snapshot cannot authorize contact with another rollout target or silently change the reviewed database/compose environment, research/zero-money boundary, or migration-headroom threshold. The helper reads the verified preflight JSON bytes once, computes the SHA-256 from that same byte snapshot, validates the JSON from those same bytes, and records that digest in the eventual rollout evidence. This prevents the recorded digest from drifting from the evidence fields actually validated if the local file changes between separate reads. It also extracts the verified archive `evidence_name`, SHA-256, and `window_end`, requires that `window_end` to be a timezone-aware ISO-8601 timestamp before any cloud contact, and carries all three into the detached worker. The worker requires that exact canonical recovery file, re-hashes its current bytes, requires the digest to equal the preflight-captured SHA-256, and then checks the matching window. A same-named recovery file whose contents changed after preflight therefore fails closed before migration.
