...
1. **Merge the index old-snapshot-wait hardening.** The first authorized index attempt reached `CREATE INDEX CONCURRENTLY`, then PostgreSQL canceled it under the helper's 5-second `lock_timeout` after the index became invalid-but-ready. Read-only diagnostics found no remaining create/vacuum progress or relation locks. Merge the helper fix that disables `lock_timeout` for concurrent DROP/CREATE while retaining the 15-minute statement timeout and 20-minute hour-headroom guard.
2. **Re-authorize the compact-feed freshness index.** The failed attempt left `ix_market_state_1s_feed_last_event` present with `indisvalid=false`, `indisready=true`, `indislive=true`. Production index cleanup/retry remains unauthorized until the hardening merge advances `main` and a new exact-current-main approval is given.
